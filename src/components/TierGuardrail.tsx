...
function CapRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-2 py-1">
      ...
      <span className="text-[10px] font-mono font-bold text-[#F7F7F8]">{value}</span>
    </div>
  );
}
...
export function TierGuardrail({ guardrail }: TierGuardrailProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#222630] bg-[#12151A] p-4 space-y-3">
      ...
      <p className="text-[10px] text-[#F7F7F8]/60 leading-snug">{guardrail.micro}</p>
      ...
      {guardrail.next && (
        <div className="rounded-xl border border-[#222630] bg-[#171A20] p-3">
          <div className="flex items-center justify-between gap-2" style={{ filter: 'blur(1.2px)', opacity: 0.65 }}>
            <span className="text-[11px] font-extrabold text-[#F7F7F8]">
              {guardrail.next.label}
            </span>
            <Lock className="h-3.5 w-3.5 text-[#F7F7F8]" />
          </div>
          <p className="mt-1 text-[10px] text-[#F7F7F8]/70 leading-snug" style={{ filter: 'blur(1.2px)', opacity: 0.7 }}>
      ...
      {attempt && (
        <div className="rounded-xl border border-[#222630] bg-[#171A20] p-2.5">
          {attempt.ok ? (
            <p className="text-[10px] text-[#F7F7F8] flex items-center gap-1.5">
              <Check className="h-3 w-3" /> Tier changed.
            </p>
          ) : (
            <>
              <p className="text-[10px] font-bold text-[#F7F7F8] leading-snug">
