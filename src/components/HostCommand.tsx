...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-xl p-3 space-y-2">
      <p className="text-[9px] text-[#111111]/40">{title}</p>
      {children}
    </div>
  );
}
...
function Big({ value, label, accent = false }: { value: string; label: string; accent?: boolean }) {
  return (
    <div>
      <p className={`text-xl font-extrabold ${accent ? 'text-[#111111]' : 'text-[#111111]'}`}>{value}</p>
...
export function HostCommand() {
  ...
  if (state.status === 'loading' && !d) {
    return <p className="text-xs text-[#111111]/60">Loading…</p>;
  }
  if (state.status === 'error') {
    return <p className="text-xs text-[#111111]">{state.error}</p>;
  }
  if (!d) return null;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-lg font-extrabold text-[#111111]">Command</h2>
        <button onClick={() => void load()} className="text-[10px] font-extrabold text-[#111111] cursor-pointer">Refresh</button>
      </div>

      {/* NOW — the one thing that matters first */}
      <Section title="Now">
        {d.now.length === 0 && d.upcoming.length === 0 ? (
          <p className="text-xs text-[#111111]/60">Nothing needs you right now.</p>
        ) : (
          <>
            {d.now.slice(0, 5).map((n, i) => (
              <div key={i} className="flex items-center justify-between gap-2">
                <p className="text-xs text-[#111111] truncate">{n.name}</p>
                <span className="shrink-0 text-[9px] text-[#111111]">unpaid spot</span>
              </div>
            ))}
            {d.upcoming.length > 0 && (
              <div className="pt-1 border-t border-[#E5E7EB]">
                <p className="text-[9px] text-[#111111]/40 mb-1">Upcoming</p>
                {d.upcoming.map((u) => (
                  <p key={u.id} className="text-xs text-[#111111] truncate">
      ...
      <Section title="Money">
        ...
        {d.money.campaignCount === 0 && (
          <p className="text-[10px] text-[#111111]/40">No campaigns yet.</p>
        )}
      </Section>
      ...
      {d.action.length > 0 && (
        <Section title="Action">
          {d.action.slice(0, 6).map((a, i) => (
            <div key={i} className="flex items-center justify-between gap-2">
              <p className="text-xs text-[#111111] truncate">{a.vaultTitle}</p>
              <span className="shrink-0 text-[9px] text-[#111111]">{a.kind}</span>
            </div>
          ))}
        </Section>
      )}
