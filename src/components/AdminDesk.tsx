...
function Card({ title, note, children }: { title: string; note?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-[#E5E8EC] bg-[#FFFFFF] p-4 space-y-2">
      ...
      {note && <p className="text-[10px] leading-snug text-[#0D1117]/60">{note}</p>}
      {children}
    </div>
  );
}
...
function Empty({ children }: { children: React.ReactNode }) {
  return <p className="text-[11px] text-[#0D1117]/70">{children}</p>;
}

function Row({ children }: { children: React.ReactNode }) {
  return <div className="flex items-center justify-between gap-2 text-[11px] text-[#0D1117]/80">{children}</div>;
}
...
function Fact({ ok, label, detail }: { ok: boolean | null; label: string; detail?: string }) {
  return (
    <Row>
      <span>{label}</span>
      <span className={`font-extrabold ${ok === false ? 'text-[#0D1117]' : 'text-[#0D1117]/60'}`}>
        {ok === null ? 'unknown' : ok ? 'yes' : 'NO'}
        {detail ? <span className="ml-1 font-bold text-[#0D1117]/60">{detail}</span> : null}
      </span>
    </Row>
  );
}
...
export function AdminDesk({ open, onClose, me }: { open: boolean; onClose: () => void; me: AuthedUser | null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#FF5A1F]/30 backdrop-blur-sm overflow-y-auto">
      <div className="max-w-3xl mx-auto min-h-full bg-[#F0F2F5]">
        <div className="sticky top-0 z-10 bg-[#F0F2F5]/95 border-b border-[#E5E8EC] px-4 pt-5 pb-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-extrabold text-[#0D1117]">Operate</h2>
            ...
            <button onClick={onClose} aria-label="Close the operator desk"
              className="shrink-0 rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] p-2 cursor-pointer">
            ...
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-extrabold cursor-pointer ${
                  tab === t.id ? 'bg-[#FF5A1F] text-[#0D1117]' : 'bg-[#FFFFFF] text-[#0D1117]/70 border border-[#E5E8EC]'
                }`}>
                {t.label}
              </button>
            ))}
        ...
        <>
                ...
                {Object.keys(funnel.funnel).length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {Object.entries(funnel.funnel).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, n]) => (
                      <span key={name} className="rounded-full bg-[#F0F2F5] px-2 py-0.5 text-[9px] font-bold text-[#0D1117]/70">{name} · {n}</span>
                    ))}
                  </div>
                )}
              </>
          ...
          <Card title="Directory" note="Search by handle or name. Suspended members are locked out on their next request, not their next login.">
            <input
              ...
              className="w-full rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[11px] outline-none focus:border-[#FF5A1F]"
            />
            {page === null ? <Empty>loading…</Empty>
              ...
              : page.rows.map((m) => (
                <button key={m.id} type="button" onClick={() => { setSelected(m); setNote(null); setReason(''); }}
                  className={`w-full rounded-xl border px-2.5 py-2 text-left cursor-pointer ${selected?.id === m.id ? 'border-[#FF5A1F] bg-[#FFFFFF]' : 'border-[#E5E8EC] bg-[#FFFFFF]'}`}>
                  <Row>
                  ...
                  <Row>
                    <span className="text-[9px] text-[#0D1117]/70">@{m.handle}</span>
          ...
          {selected && (
            <Card title={`@${selected.handle}`} note="Roles and suspension are audited with before/after. A suspension needs a reason.">
              ...
              <div className="pt-1">
                <p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#0D1117]/60 pt-1">Platform roles</p>
                <div className="flex flex-wrap gap-1 pt-1">
                    ...
                    <button key={role} type="button" onClick={() => toggleRole(selected, role)}
                      className={`rounded-full px-2 py-0.5 text-[9px] font-extrabold cursor-pointer border ${
                        selected.platformRoles.includes(role)
                          ? 'bg-[#FF5A1F] text-[#0D1117] border-[#FF5A1F]'
              ...
              {selected.status === 'active' ? (
                <div className="space-y-1.5 pt-2">
                  <input value={reason} onChange={(e) => setReason(e.target.value)}
                    ...
                    className="w-full rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[10px] outline-none focus:border-[#FF5A1F]" />
      ...
      <Card title="Recent errors" note="Where silent breakage hides. Empty means none recorded.">
        {errors.length === 0 ? <Empty>No recorded errors.</Empty> : errors.slice(-8).reverse().map((e, i) => (
          <Row key={e.id ?? i}>
            ...
            <span className="shrink-0 text-[9px] text-[#0D1117]/60">{String(e.at ?? e.createdAt ?? '').slice(0, 16)}</span>
          </Row>
        ))}
      </Card>
      {can('ops.run') ? (
        <button onClick={() => void backup()}
          className="w-full rounded-xl bg-[#FF5A1F] px-3 py-2.5 text-[11px] font-extrabold text-[#0D1117] cursor-pointer">
    ...
    <>
      ...
      <Card title="Reports on objects" note="Flag → inspect → decide. Removal hides without deleting; every decision is audited with a reason.">
        {reports === null ? <Empty>loading…</Empty>
          ...
          : reports.map((r) => (
            <div key={r.id} className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1">
              <Row><span>{String(r.reason ?? r.kind ?? 'report')}</span><span className="text-[9px] text-[#0D1117]/60">{String(r.createdAt ?? '').slice(0, 10)}</span></Row>
              ...
              {can('moderate') ? (
                <div className="flex gap-1.5 pt-1">
                  ...
                  <button onClick={() => void act(() => briefApi.resolveOpsReport(r.id, 'remove', 'removed from discovery at the desk'), 'removed from discovery')}
                    className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D1117] cursor-pointer">Remove from discovery</button>
                </div>
      ...
      <Card title="Corrections" note="Fix a bad extracted field. The ORIGINAL source value is preserved on the correction row; provenance is never rewritten. Both apply and reject are audited.">
        {corrections === null ? <Empty>loading…</Empty>
          ...
          : corrections.map((c) => (
            <div key={c.id} className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1">
              <Row>
              ...
              {c.status === 'applied' && can('moderate') && (
                <div className="flex gap-1.5 pt-1">
                  <button onClick={() => void act(() => briefApi.opsRejectCorrection(c.id, 'rejected at the desk'), 'correction rejected')}
                    className="rounded-lg border border-[#E5E8EC] px-2 py-1 text-[9px] font-extrabold cursor-pointer">Reject</button>
                </div>
              )}
            </div>
          ))}
        {can('moderate') && (
          <div className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1.5">
            <p className="text-[10px] font-extrabold text-[#0D1117]/60">Correct a field</p>
            <div className="flex flex-wrap gap-1.5">
              <input
                value={corrObjectId}
                onChange={(e) => setCorrObjectId(e.target.value)}
                placeholder="object id"
                className="min-w-0 flex-1 rounded-lg border border-[#E5E8EC] px-2 py-1 text-[10px]"
              />
              <select value={corrField} onChange={(e) => setCorrField(e.target.value)} className="rounded-lg border border-[#E5E8EC] px-2 py-1 text-[10px]">
              ...
              <button
                ...
                className="rounded-lg bg-[#FF5A1F] px-2.5 py-1 text-[9px] font-extrabold text-[#0D1117] cursor-pointer"
              >
                Apply
              </button>
            </div>
      ...
      <Card title="Source trust" note="An operator decision about a source's standing — never a public rating, never 'this source is always true'. Degraded ranks lower; disabled stops contributing to the default feed (content stays reachable directly).">
        {trustSources === null ? <Empty>loading…</Empty>
          ...
          : trustSources.map((s) => (
            <div key={s.id} className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1">
              <Row>
                <span className="font-extrabold">{String(s.name)}</span>
                <span className="text-[9px] font-extrabold uppercase text-[#0D1117]/60">{String(s.trustStatus)}</span>
              </Row>
              <p className="text-[10px] text-[#0D1117]/70">
                  ...
                  {(['trusted', 'normal', 'degraded', 'disabled'] as const).map((st) => (
                    <button key={st} onClick={() => void act(() => briefApi.opsSetSourceTrust(s.id, st, `set to ${st} at the desk`), `source ${st}`)}
                      className={`rounded-lg border px-2 py-1 text-[9px] font-extrabold cursor-pointer ${
                        s.trustStatus === st ? 'bg-[#FF5A1F] border-[#FF5A1F] text-[#0D1117]' : 'border-[#E5E8EC] text-[#0D1117]/70'
                      }`}>
                      {st}
                    </button>
                  ))}
          ...
          : queue.map((r) => (
            <div key={r.id} className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1">
              <Row>
                ...
                <span className="text-[9px] text-[#0D1117]/60">submitted {String(r.submittedAt ?? '').slice(0, 10)}</span>
              </Row>
              <p className="text-[10px] text-[#0D1117]/70">user {String(r.userId ?? '?')}{r.note ? ` — “${String(r.note)}”` : ''}</p>
        ...
        {disputes === null ? <Empty>loading…</Empty>
          ...
          : disputes.slice(0, 10).map((d) => (
            <Row key={d.id}>
              ...
              <span className="shrink-0 text-[9px] text-[#0D1117]/60">order {String(d.orderId ?? '?').slice(0, 12)}</span>
            </Row>
          ))}
      </Card>
        ...
        {listings === null ? <Empty>loading…</Empty>
          ...
          : listings.slice(0, 12).map((l) => (
            <Row key={l.id}>
              ...
              <span className="shrink-0 text-[9px] text-[#0D1117]/60">KES {String(l.priceKes ?? l.price ?? '?')}</span>
            </Row>
          ))}
      </Card>
    </>
    ...
    <div className="space-y-1.5 pt-1">
      ...
      <div className="flex gap-1.5">
        <button onClick={() => onDecide('approved', reason || 'approved at the desk')}
          className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D1117] cursor-pointer">Approve</button>
    ...
    <>
      ...
      <Card title="Ingest status" note="What the sources layer reports about itself right now.">
        {ingest === null ? <Empty>loading…</Empty> : (
          <pre className="overflow-x-auto rounded-xl bg-[#FF5A1F] px-3 py-2 text-[9px] leading-relaxed text-[#0D1117]/90">
    ...
    <>
      ...
      <Card title="Editorial collections" note="The curated shelves. Transitions follow the collection state machine; moderation is audited.">
        {cols === null ? <Empty>loading…</Empty>
          ...
          : cols.map((c) => (
            <div key={c.id ?? c.key} className="rounded-xl border border-[#E5E8EC] p-2.5 space-y-1">
              <Row><span className="font-extrabold">{String(c.title ?? c.key)}</span><span className="text-[9px] text-[#0D1117]/60">{String(c.status ?? '')}</span></Row>
              {can('moderate') && (
                <div className="flex gap-1.5 pt-1">
                  {['publish', 'archive'].map((a) => (
                    <button key={a} onClick={() => void transition(String(c.key), a)}
                      className="rounded-lg border border-[#E5E8EC] px-2 py-1 text-[9px] font-extrabold cursor-pointer">{a}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        {!can('admin') ? <Empty>Seeding needs the admin capability.</Empty> : (
          <div className="flex gap-1.5">
            ...
            <button onClick={async () => { const r = await briefApi.clearDemo(); setNote(r.ok ? 'demo content cleared' : r.error); refresh(); }}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D1117] cursor-pointer">Clear demo</button>
          </div>
        )}
      </Card>
    </>
        ...
        <Card title="Record an editorial image" note="Validation is the server's: a bad kind or key is refused with the reason.">
          <div className="space-y-1.5">
            ...
            <button onClick={() => void record()}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1.5 text-[9px] font-extrabold text-[#0D1117] cursor-pointer">Record</button>
          </div>
        </Card>
        ...
        {settle === null ? <Empty>loading…</Empty> : (
          <>
            ...
            {(settle.discrepancies ?? []).slice(0, 8).map((d: any, i: number) => (
              <Row key={i}><span className="text-[#0D1117]">{String(d.kind)}</span><span className="text-[9px] text-[#0D1117]/60">{String(d.orderId ?? d.intentId ?? '')}</span></Row>
            ))}
          </>
        )}
      </Card>
        ...
        {pay === null ? <Empty>loading…</Empty> : (
          <>
            ...
            {(pay.discrepancies ?? []).slice(0, 8).map((d: any, i: number) => (
              <Row key={i}><span className="text-[#0D1117]">{String(d.kind)}</span><span className="text-[9px] text-[#0D1117]/60">{String(d.intentId ?? '')}</span></Row>
            ))}
            {(pay.discrepancies ?? []).length === 0 && <Empty>No intent discrepancies.</Empty>}
          </>
        )}
      </Card>
      <button onClick={refresh} className="w-full rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] px-3 py-2 text-[11px] font-extrabold cursor-pointer">
        ...
        {!can('admin') ? <Empty>Assigning roles needs the admin capability.</Empty> : (
          <div className="space-y-1.5">
            <input value={roles.userId} onChange={(e) => setRoles({ ...roles, userId: e.target.value })}
              placeholder="user id or handle" className="w-full rounded-lg border border-[#E5E8EC] px-2 py-1.5 text-[10px]" />
            <input value={roles.roles} onChange={(e) => setRoles({ ...roles, roles: e.target.value })}
              placeholder="roles (viewer, operator, reviewer, finance, admin)" className="w-full rounded-lg border border-[#E5E8EC] px-2 py-1.5 text-[10px]" />
            ...
            <button onClick={() => void submit()} disabled={!roles.userId.trim() || !roles.reason.trim()}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1.5 text-[9px] font-extrabold text-[#0D1117] cursor-pointer disabled:opacity-40">
              Write roles
            </button>
          </div>
        )}
        ...
        {audit === null ? <Empty>loading…</Empty>
          ...
          : audit.slice(0, 15).map((a, i) => (
            <Row key={a.id ?? i}>
              ...
              <span className="shrink-0 text-[9px] text-[#0D1117]/60">{String(a.at ?? a.createdAt ?? '').slice(0, 16)}</span>
            </Row>
          ))}
      </Card>
        ...
        {mailLog === null ? <Empty>loading…</Empty>
          ...
          : mailLog.map((m, i) => (
            <Row key={m.id ?? i}>
              ...
              <span className="shrink-0 text-[9px] text-[#0D1117]/60">{String(m.at ?? m.createdAt ?? '').slice(0, 16)}</span>
            </Row>
          ))}
      </Card>
    </>
