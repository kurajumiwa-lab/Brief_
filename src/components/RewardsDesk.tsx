...
export default function RewardsDesk({ settledPoints, rank, accepted, pending }: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
      <div>
        <h2 className="text-lg font-extrabold text-[#251045]">Rewards</h2>
      ...
      <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-4 space-y-2">
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[10px] text-[#251045]/40">Contribution Points</span>
          <span className="text-lg font-extrabold text-[#251045]">{settledPoints.toLocaleString()}</span>
        </div>
        <p className="text-[10px] text-[#251045]/60">Progress {rank} - {accepted} accepted contributions</p>
        {pending > 0 && <p className="text-[10px] text-[#251045]">{pending} submitted, awaiting review. Worth nothing yet.</p>}
      </div>

      {data && (
        <>
          <section aria-label="Your referral code" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-2">
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-[#5B2EA6]" aria-hidden="true" />
              <h3 className="text-[13px] font-extrabold text-[#251045]">Your code</h3>
            </div>
            ...
            {shareMsg && (
              <p className="text-[9px] text-[#251045]/60 whitespace-pre-line border border-[#D6CFE4] rounded-xl p-2">{shareMsg}</p>
            )}
          </section>

          <section aria-label="Points balance" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-2">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-[10px] text-[#251045]/40">Referral points</span>
              <span className="text-lg font-extrabold text-[#251045]">{data.balance.available.toLocaleString()}</span>
            </div>
            ...
            {note && <p className="text-[11px] font-bold text-[#251045]" role="status">{note}</p>}
          </section>

          {data.conversions.length > 0 && (
            <section aria-label="Payouts" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-2">
              <h3 className="text-[13px] font-extrabold text-[#251045]">Payouts</h3>
              {data.conversions.slice(0, 8).map((c) => (
                <div key={c.id} className="rounded-xl border border-[#D6CFE4] px-3 py-2">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[11px] font-bold text-[#251045]">{c.points.toLocaleString()} points → KES {c.kes.toLocaleString()}</p>
                    <p className={`text-[10px] font-extrabold uppercase ${c.status === 'confirmed' ? 'text-[#2E6B3F]' : c.status === 'refused' ? 'text-[#8A3B3B]' : 'text-[#251045]/60'}`}>{c.status}</p>
                  </div>
                  {c.status === 'refused' && c.refusedReason && <p className="text-[9px] text-[#8A3B3B]">Refused: {c.refusedReason}</p>}
                </div>
              ))}
            </section>
          )}
          ...
          {data.events.length > 0 && (
            <section aria-label="How your points were earned" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-1.5">
              <h3 className="text-[13px] font-extrabold text-[#251045]">How they were earned</h3>
              {data.events.slice(0, 10).map((e) => (
                <p key={e.id} className="text-[10px] text-[#251045]/70">
