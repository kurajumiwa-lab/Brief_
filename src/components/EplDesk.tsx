...
const LOBBY_STYLE: Record<string, string> = {
  waiting_for_players: 'bg-[#E5E7EB] text-[#111111]/60',
  open: 'bg-[#111111] text-[#FFFFFF]',
  full: 'bg-[#E5E7EB] text-[#111111]',
  in_progress: 'bg-[#111111] text-[#FFFFFF]',
  completed: 'bg-[#FFFFFF] text-[#111111]',
  cancelled: 'bg-[#E5E7EB] text-[#111111]/40'
};
...
export function EplDesk({ meId, onToast }: { meId: string | null; onToast: (msg: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-extrabold text-[#111111]">EPL contest rooms</h2>
      ...
      {/* create a room */}
      <div className="rounded-2xl border border-[#E5E7EB] bg-[#FFFFFF] p-3.5 space-y-2">
        <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#111111]">Open a room</p>
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className="min-w-[160px] flex-1 rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] px-3 py-2 text-[12px] text-[#111111]"
          />
          <input
            type="datetime-local"
            value={kickoff}
            ...
            aria-label="kickoff"
            className="rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] px-2.5 py-2 text-[12px] text-[#111111]"
          />
          <input
            value={budget}
            ...
            className="w-28 rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] px-3 py-2 text-[12px] text-[#111111]"
          />
          <input
            value={minEntries}
        ...
        {formNote && <p className="text-[11px] text-[#111111]">{formNote}</p>}
      </div>

      {error && <p className="text-xs text-[#111111]">{error}</p>}
      {rooms === null && <p className="text-xs text-[#111111]/60">Loading rooms…</p>}
      {rooms !== null && rooms.length === 0 && !error && (
        <p className="text-xs text-[#111111]/60">No rooms yet. Open one — the waiting room is honest about being empty.</p>
      )}
      ...
      <div className="space-y-2">
        {(rooms ?? []).map((r) => (
          <div
            key={r.id}
            className={`rounded-2xl border bg-[#FFFFFF] p-3.5 space-y-2 ${selectedId === r.id ? 'border-[#111111]' : 'border-[#E5E7EB]'}`}
          >
            <button
              type="button"
              ...
              <div className="space-y-2 border-t border-[#E5E7EB] pt-2">
                {r.mine && (
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      disabled={busy}
                      className="rounded-lg border border-[#111111] px-3 py-1.5 text-[10px] font-extrabold text-[#111111] cursor-pointer disabled:opacity-40"
                    >
                      Import catalog pool
                    </button>
                    <button
                      ...
                      disabled={busy}
                      className="rounded-lg bg-[#111111] px-3 py-1.5 text-[10px] font-extrabold text-[#FFFFFF] cursor-pointer disabled:opacity-40"
                    >
                ...
                {pool !== null && pool.length > 0 && r.status !== 'cancelled' && (
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[10px] text-[#111111]/60">
                      <span>Pick 11 (1 GK · 3+ DEF · 2+ MID · 1+ FWD · max 3 per club)</span>
                      <span className={spend > (r.budgetKes ?? Infinity) ? 'font-extrabold text-[#111111]' : ''}>
                      ...
                      {pool.slice(0, 60).map((p) => {
                        ...
                        return (
                          <button
                            ...
                            className={`text-left rounded-lg border px-2 py-1.5 cursor-pointer ${picked ? 'border-[#111111] bg-[#111111] text-[#FFFFFF]' : 'border-[#E5E7EB] bg-[#FAFAFA] text-[#111111]'}`}
                          >
                            <p className="text-[10px] font-extrabold truncate">{p.name}</p>
                            <p className={`text-[9px] ${picked ? 'text-[#FFFFFF]/70' : 'text-[#111111]/40'}`}>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <select
                        ...
                        aria-label="captain"
                        className="rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] px-2.5 py-2 text-[11px] text-[#111111]"
                      >
                    ...
                    {seatNote && <p className="text-[10px] text-[#111111] break-words">{seatNote}</p>}
                    <p className="text-[9px] leading-snug text-[#111111]/40">
                ...
                {standings !== null && standings.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-[9px] font-extrabold uppercase tracking-[0.14em] text-[#111111]/40">Standings</p>
                    {standings.map((s) => (
                      <div key={s.userId} className="flex items-center justify-between text-[10px] text-[#111111]">
