...
export function EventResale({ slug }: { slug: string }) {
  ...
  if (load.status === 'loading') {
    return (
      <div className="border-t border-[#D6CFE4] pt-4">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#251045]/50 mb-2">Resale tickets</h3>
        <p className="text-xs text-[#251045]/50 py-2">Checking for resale seats…</p>
      </div>
    );
  }
  ...
  if (load.status === 'error') {
    return (
      <div className="border-t border-[#D6CFE4] pt-4 space-y-2">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#251045]/50">Resale tickets</h3>
        <p className="text-[11px] text-[#251045]/70">Resale seats could not be loaded right now. {load.error}</p>
        <button onClick={() => void fetchListings()} className="text-xs font-bold text-[#251045] underline cursor-pointer">
          Try again
        </button>
      </div>
    );
  ...
  return (
    <div className="border-t border-[#D6CFE4] pt-4 space-y-3">
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#251045]/50">Resale tickets</h3>
        <span className="text-[10px] text-[#251045]/40">{load.data?.length ?? 0} listed</span>
      </div>
        ...
        <div className="border border-[#6C3EC9] rounded-2xl p-4 space-y-2 bg-[#F4F1FA]">
          ...
          <div className="flex gap-2">
            <button
              disabled={busy}
              onClick={() => void cancel()}
              className="text-xs px-3 py-2 rounded-xl border border-[#D6CFE4] bg-[#FBFAFD] text-[#251045] cursor-pointer disabled:opacity-40"
            >
              Let it go
            </button>
          </div>
      ...
      {error && <p className="text-[11px] text-[#251045]">{error}</p>}

      <div className="space-y-2">
        {load.data?.map((l) => (
          <div key={l.id} className="border border-[#D6CFE4] rounded-2xl p-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-extrabold text-[#251045]">
                {money(l.price, l.currency)} {l.cheapest && <span className="text-[9px] font-bold text-[#251045]/50">· cheapest</span>}
              </p>
              <p className="text-[10px] text-[#251045]/50 truncate">
              ...
              {l.note && <p className="text-[10px] text-[#251045]/60 mt-0.5">“{l.note}”</p>}
            </div>
            <button
              disabled={busy || Boolean(order)}
              onClick={() => void buy(l.id)}
              className="shrink-0 text-xs font-bold px-3 py-2 rounded-xl bg-[#5B2EA6] text-[#FFFFFF] cursor-pointer disabled:opacity-40"
            >
              Buy
            </button>
          </div>
