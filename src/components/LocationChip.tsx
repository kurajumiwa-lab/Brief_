  ...
  return (
    <div className="relative">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="flex items-center gap-1.5 bg-[#FFFFFF] text-[#111111] text-xs font-bold px-3 py-1.5 rounded-full border border-[#E5E7EB] hover:border-[#111111] transition-colors cursor-pointer"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-2 z-50 w-64 rounded-xl border border-[#E5E7EB] bg-[#FFFFFF] p-2 shadow-2xl">
            <button
              onClick={() => { onLocate(); }}
              className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-[#111111] hover:bg-[#F3F4F6] cursor-pointer"
            >
              <Compass className="w-3.5 h-3.5 text-[#111111]" />
              {locating ? 'Locating…' : 'Use my location'}
            </button>
            {locError && <p className="px-3 py-1 text-[10px] text-[#111111]">{locError}</p>}

            <div className="my-1 h-px bg-[#E5E7EB]" />
            <p className="px-3 py-1 text-[10px] uppercase tracking-[0.08em] text-[#111111]/60">Or tap a city</p>
            <div className="flex flex-wrap gap-1 px-2 pb-1">
              {CITIES.map((c) => (
                <button
                  key={c.label}
                  onClick={() => { onSelectCity(c); setOpen(false); }}
                  className="rounded-full border border-[#E5E7EB] px-3 py-1 text-[11px] font-semibold text-[#111111]/60 hover:border-[#111111] hover:text-[#111111] cursor-pointer"
                >
                  {c.label}
                </button>
              ))}
            ...
            {hasLocation && (
              <>
                ...
                <button
                  ...
                  className="w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-[#111111]/60 hover:bg-[#F3F4F6] cursor-pointer"
                >
                  Show everywhere
                </button>
              </>
