...
export function NextStep({ ladder, onAct, onDismiss, compact = false }: NextStepProps) {
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-[#111111] bg-[#FFFFFF] px-4 py-3.5 shadow-sm"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-0.5 text-[14px] font-extrabold tracking-tight text-[#111111]">{next.label}</h3>
          {!compact && <p className="mt-1 text-[11px] leading-snug text-[#111111]/60">{next.detail}</p>}
        </div>
        {onDismiss && (
          <button
            type="button"
            onClick={onDismiss}
            className="shrink-0 text-[9px] font-extrabold uppercase tracking-[0.14em] text-[#111111]/40 hover:text-[#111111] cursor-pointer"
          >
            Hide
          </button>
        )}
      ...
      <div className="mt-2.5 flex items-center gap-1">
        {ladder.rungs.map((rung) => (
          <span
            ...
            className={`h-1.5 flex-1 rounded-full ${rung.reached ? 'bg-[#111111]' : 'bg-[#E5E7EB]'}`}
          />
        ))}
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl bg-[#111111] px-3.5 py-2 text-[11px] font-extrabold text-[#FFFFFF] cursor-pointer"
        >
          {next.cta} <ArrowRight className="h-3 w-3" />
        </button>
        {nextService && (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#111111]/50">
      ...
      {!compact && ladder.reached.length > 0 && (
        <p className="mt-2 flex items-center gap-1 text-[10px] text-[#111111]/45">
