...
export default function ServiceFees() {
  ...
  const statusChip = (f: ServiceFee) =>
    f.status === 'confirmed' ? 'text-[#2E6B3F]'
      : f.status === 'refused' ? 'text-[#8A3B3B]'
        : 'text-[#251045]/60';

  return (
    <div className="space-y-4">
      <section aria-label="Pay for Brief services" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-3">
        <div className="flex items-center gap-2">
          <Wallet className="w-4 h-4 text-[#5B2EA6]" aria-hidden="true" />
          <h2 className="text-[13px] font-extrabold text-[#251045]">Pay for Brief services</h2>
        </div>
        ...
        <div className="space-y-2">
          {(data?.services ?? []).map((svcItem) => (
            <label key={svcItem.key} className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-2 cursor-pointer ${service === svcItem.key ? 'border-[#5B2EA6] bg-[#F1EDF7]' : 'border-[#D6CFE4]'}`}>
          ...
          {data && data.services.length === 0 && (
            <p className="text-[11px] text-[#251045]/60">No services are priced yet.</p>
          )}
        </div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-[#D6CFE4] bg-[#F1EDF7] px-3 py-2 text-[12px] tracking-wide text-[#251045]"
          />
      ...
      <section aria-label="Your payments" className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-2">
        <h2 className="text-[13px] font-extrabold text-[#251045]">Your payments</h2>
        {(data?.fees ?? []).length === 0 && (
          <p className="text-[11px] text-[#251045]/60">Nothing yet. Your first payment will show here, with its state.</p>
        )}
        {(data?.fees ?? []).slice(0, 10).map((f) => (
          <div key={f.id} className="rounded-xl border border-[#D6CFE4] px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[11px] font-bold text-[#251045]">{f.label}</p>
            ...
            {f.status === 'refused' && f.refusedReason && (
              <p className="text-[9px] text-[#8A3B3B]">Refused: {f.refusedReason}</p>
            )}
          </div>
        ))}
      </section>
