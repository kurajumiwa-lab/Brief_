...
export function BroadcastRail({
  ...
  return (
    <section className="space-y-2" aria-label="Broadcasts to followers">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {broadcasts.map((b) => (
            ...
            >
              ...
              <p className="text-[12px] font-medium leading-snug mt-1" style={{ color: '#0A0A0A' }}>
                {b.text}
              </p>
              <p className="text-[10px] font-mono mt-2 inline-flex items-center gap-1" style={{ color: '#6B7280' }}>
      ...
      {open && (
        <form onSubmit={send} className="p-3 rounded-2xl space-y-2" style={{ background: '#F4F4F7' }}>
          <div className="flex flex-wrap gap-1.5">
            {KINDS.map((k) => (
              <button
                ...
                style={{
                  background: kind === k.id ? 'var(--color-primary)' : '#fff',
                  color: kind === k.id ? 'var(--accent-ink)' : '#6B7280',
                  borderColor: kind === k.id ? 'transparent' : '#E5E7EB'
                }}
                title={k.hint}
              >
                {k.label}
          ...
          <textarea
            ...
            className="w-full px-3 py-2 rounded-xl text-[13px] border bg-white resize-none"
