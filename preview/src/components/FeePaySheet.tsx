...
export function FeePaySheet({ title, intro, serviceKeys, target, afterSubmit, onClose, onPaid }: FeePaySheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-[#251045]/40 p-0 sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label={title}>
      <div className="w-full max-w-md rounded-t-3xl border border-[#D6CFE4] bg-[#FBFAFD] p-5 shadow-2xl sm:rounded-3xl">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Wallet className="h-4 w-4 text-[#5B2EA6]" aria-hidden="true" />
            <h2 className="text-[13px] font-extrabold text-[#251045]">{title}</h2>
          </div>
          <button type="button" onClick={onClose} aria-label="Close" className="cursor-pointer rounded-full p-1 text-[#251045]/50 hover:bg-[#F1EDF7]">
            <X className="h-4 w-4" aria-hidden="true" />
          </button>
        </div>

        <p className="mb-3 text-[11px] leading-relaxed text-[#251045]/70">{intro}</p>

        {unavailable && (
          <p className="rounded-xl border border-[#D6CFE4] bg-[#FFFFFF] p-3 text-[11px] font-semibold text-[#8A3B3B]">
            Payments are unavailable right now: {unavailable}
          </p>
        )}

        {!unavailable && offered.length === 0 && (
          <p className="rounded-xl border border-[#D6CFE4] bg-[#FFFFFF] p-3 text-[11px] font-semibold text-[#251045]/60">
        ...
        {offered.length > 0 && (
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-[#251045]/70">
              {data?.pochi
                ? <>Pay with M-PESA to <span className="font-bold text-[#251045]">Pochi la Biashara {data.pochi}</span>, then paste the confirmation code below.</>
                : 'Pay to the Brief Pochi la Biashara number (ask for it if you do not have it), then paste the confirmation code below.'}
            </p>

            {offered.map((svcItem) => (
              <label key={svcItem.key} className={`flex cursor-pointer items-center justify-between gap-2 rounded-xl border px-3 py-2 ${service === svcItem.key ? 'border-[#5B2EA6] bg-[#F1EDF7]' : 'border-[#D6CFE4]'}`}>
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-[#251045]">{svcItem.label}</span>
                </span>
                {/* The price comes from the server catalog — never from this file. */}
                <span className="text-[11px] font-extrabold text-[#5B2EA6]">KES {svcItem.amountKes}</span>
              </label>
            ))}
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#D6CFE4] bg-[#FFFFFF] px-3 py-2 text-[12px] font-semibold text-[#251045] outline-none placeholder:text-[#251045]/35 focus:border-[#6C3EC9]"
            />
            ...
            <button
              ...
              className="w-full cursor-pointer rounded-xl bg-[#5B2EA6] py-2.5 text-[12px] font-extrabold text-[#FFFFFF] disabled:cursor-not-allowed disabled:opacity-40"
            >
              {busy ? 'Recording…' : 'Submit confirmation code'}
            </button>

            {note && <p className="text-[11px] font-semibold leading-relaxed text-[#251045]/70">{note}</p>}
          </div>
        )}
      </div>
    </div>
