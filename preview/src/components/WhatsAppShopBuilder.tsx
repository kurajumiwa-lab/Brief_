        ...
        <>
          ...
          <div className="flex items-center gap-1.5 pt-1 border-t border-[#E5E8EC]">
            <input list="book-items" value={name} onChange={(e) => setName(e.target.value)} placeholder="What sold?" maxLength={60}
              className="flex-1 min-w-0 rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[12px] font-bold text-[#0D1117] outline-none focus:border-[#4F46E5]" />
            ...
            <input value={qty} onChange={(e) => setQty(e.target.value.replace(/[^\d]/g, ''))} placeholder="1" inputMode="numeric"
              className="w-12 rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2 py-1.5 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" aria-label="quantity" />
            <button type="button" onClick={() => void log()} disabled={busy || !name.trim()}
              className="rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[11px] font-extrabold text-[#0D1117] disabled:opacity-40 cursor-pointer">Log</button>
          </div>
    ...
    <section aria-label="Pool a restock" className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4 space-y-2.5">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <input list="book-items" value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="Item (from your list)" maxLength={60}
          className="col-span-2 rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[12px] font-bold text-[#0D1117] outline-none focus:border-[#4F46E5]" />
        <input value={unitCost} onChange={(e) => setUnitCost(e.target.value.replace(/[^\d]/g, ''))} placeholder="Bulk cost/unit (KES)" inputMode="numeric"
          className="rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" aria-label="bulk unit cost in shillings" />
        <input value={goal} onChange={(e) => setGoal(e.target.value.replace(/[^\d]/g, ''))} placeholder="Goal (units)" inputMode="numeric"
          className="rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" aria-label="goal units" />
        <input value={mine} onChange={(e) => setMine(e.target.value.replace(/[^\d]/g, ''))} placeholder="Your units" inputMode="numeric"
          className="rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2.5 py-1.5 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" aria-label="your pledged units" />
        <button type="button" onClick={() => void pool()} disabled={busy || !itemName || !unitCost || !goal || !mine}
          className="rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[11px] font-extrabold text-[#0D1117] disabled:opacity-40 cursor-pointer">Open the pool</button>
      </div>
      ...
      {out && (
        <div className="rounded-xl bg-[#EFF1F4] border border-[#E5E8EC] p-2.5 space-y-1.5">
          ...
          <a href={out.waMe} target="_blank" rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[10.5px] font-extrabold text-[#0D1117] no-underline">
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
      <header className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          {published && (
            <span className="px-2 py-0.5 rounded-full bg-[#4F46E5] text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#0D1117]">Live</span>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4 space-y-3">
            <label className="block space-y-1">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} maxLength={40} placeholder="Mama Njeria Fresh"
                className="w-full rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] px-3 py-2 text-[13px] font-bold text-[#0D1117] outline-none focus:border-[#4F46E5]" />
            </label>
            <label className="block space-y-1">
              ...
              <input value={tagline} onChange={(e) => setTagline(e.target.value)} maxLength={60} placeholder="Fresh groceries, Kilimani"
                className="w-full rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] px-3 py-2 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" />
            </label>
            <label className="block space-y-1">
              ...
              <input value={orderNumber} onChange={(e) => setOrderNumber(e.target.value)} maxLength={20} placeholder="+254 712 345 678" inputMode="tel"
                className="w-full rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] px-3 py-2 text-[13px] text-[#0D1117] outline-none focus:border-[#4F46E5]" />
            </label>
          </div>
          ...
          <div className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4 space-y-2">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-1.5">
                ...
                <input value={item.priceKes} onChange={(e) => setItems(items.map((it, i) => i === idx ? { ...it, priceKes: e.target.value.replace(/[^\d]/g, '') } : it))} placeholder="50" inputMode="numeric"
                  className="w-16 rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2 py-1.5 text-[12px] text-[#0D1117] outline-none focus:border-[#4F46E5]" aria-label={`price of item ${idx + 1} in shillings`} />
                <input value={item.note} onChange={(e) => setItems(items.map((it, i) => i === idx ? { ...it, note: e.target.value } : it))} placeholder="note (optional)" maxLength={40}
                  className="w-24 rounded-lg border border-[#E5E8EC] bg-[#FFFFFF] px-2 py-1.5 text-[11px] text-[#0D1117]/70 outline-none focus:border-[#4F46E5]" aria-label={`note on item ${idx + 1}`} />
            ...
            <button type="button" onClick={() => setItems([...items, { name: '', priceKes: '', note: '' }])} disabled={items.length >= 40}
              className="w-full rounded-xl border border-dashed border-[#E5E8EC] py-1.5 text-[11px] font-extrabold text-[#4F46E5] hover:border-[#4F46E5] disabled:opacity-40 cursor-pointer flex items-center justify-center gap-1">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => void save()} disabled={busy}
              className="rounded-xl bg-[#4F46E5] px-4 py-2 text-[12px] font-extrabold text-[#0D1117] disabled:opacity-40 cursor-pointer">Save</button>
            {published ? (
              ...
              <button type="button" onClick={() => void publish()} disabled={busy}
                className="rounded-xl border border-[#4F46E5] bg-[#FFFFFF] px-4 py-2 text-[12px] font-extrabold text-[#4F46E5] disabled:opacity-40 cursor-pointer">Publish</button>
            )}
          </div>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => void copyText()}
                  className="rounded-xl bg-[#4F46E5] px-3.5 py-2 text-[11px] font-extrabold text-[#0D1117] cursor-pointer flex items-center gap-1.5">
