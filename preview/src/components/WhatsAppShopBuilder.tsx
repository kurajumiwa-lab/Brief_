...
function TheBook({ items, onQueued }: { items: { name: string; priceKes: string }[]; onQueued?: (n: number) => void }) {
  ...
  return (
    <section aria-label="The book" className="bg-[color:var(--color-paper)] border border-[var(--brief-line)] rounded-2xl p-4 space-y-3">
      <div className="flex items-center gap-2">
        <BookOpen className="w-4 h-4 text-[#4F46E5]" aria-hidden="true" />
        <h3 className="text-[13px] font-extrabold text-[var(--brief-ink)]">The book</h3>
        {book && <span className="text-[9px] text-[var(--ink-60)] ml-auto">derived, never stored</span>}
      </div>
      ...
      {book ? (
        <>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={label} className="rounded-xl bg-[color:var(--color-well)] px-2.5 py-2">
                <p className="text-[9px] font-bold uppercase tracking-[0.1em] text-[var(--ink-70)]">{label}</p>
                <p className="text-[13px] font-extrabold text-[var(--brief-ink)] mt-0.5 truncate">{kes(d.kes)}</p>
                <p className="text-[9px] text-[var(--ink-55)]">{d.sales} sale{d.sales === 1 ? '' : 's'} · {d.items} item{d.items === 1 ? '' : 's'}</p>
              </div>
            ))}
          </div>
          ...
          {book.topItems.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {book.topItems.map((t, i) => (
                <span key={t.name} className="px-2 py-0.5 rounded-full bg-[color:var(--color-well)] text-[10px] font-bold text-[var(--ink-70)]">
                  {i === 0 ? '🔥 ' : ''}{t.name} ×{t.qty}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-1.5 pt-1 border-t border-[var(--brief-line)]">
            <input list="book-items" value={name} onChange={(e) => setName(e.target.value)} placeholder="What sold?" maxLength={60}
              className="flex-1 min-w-0 rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2.5 py-1.5 text-[12px] font-bold text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" />
            ...
            <input value={qty} onChange={(e) => setQty(e.target.value.replace(/[^\d]/g, ''))} placeholder="1" inputMode="numeric"
              className="w-12 rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2 py-1.5 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" aria-label="quantity" />
            <button type="button" onClick={() => void log()} disabled={busy || !name.trim()}
              className="rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[11px] font-extrabold text-[var(--accent-ink)] disabled:opacity-40 cursor-pointer">Log</button>
          </div>
        ...
        <p className="text-[11px] text-[var(--ink-55)]">Save the shop and the book opens — today, the week, what is moving, what is low.</p>
      )}
    </section>
  );
}
...
function PoolACard({ items }: { items: { name: string; priceKes: string }[] }) {
  ...
  return (
    <section aria-label="Pool a restock" className="bg-[color:var(--color-paper)] border border-[var(--brief-line)] rounded-2xl p-4 space-y-2.5">
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-[13px] font-extrabold text-[var(--brief-ink)]">Pool a restock</h3>
        <span className="text-[9px] text-[var(--ink-60)]">bulk price, pooled demand</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <input list="book-items" value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="Item (from your list)" maxLength={60}
          className="col-span-2 rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2.5 py-1.5 text-[12px] font-bold text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" />
        <input value={unitCost} onChange={(e) => setUnitCost(e.target.value.replace(/[^\d]/g, ''))} placeholder="Bulk cost/unit (KES)" inputMode="numeric"
          className="rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2.5 py-1.5 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" aria-label="bulk unit cost in shillings" />
        <input value={goal} onChange={(e) => setGoal(e.target.value.replace(/[^\d]/g, ''))} placeholder="Goal (units)" inputMode="numeric"
          className="rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2.5 py-1.5 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" aria-label="goal units" />
        <input value={mine} onChange={(e) => setMine(e.target.value.replace(/[^\d]/g, ''))} placeholder="Your units" inputMode="numeric"
          className="rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2.5 py-1.5 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" aria-label="your pledged units" />
        <button type="button" onClick={() => void pool()} disabled={busy || !itemName || !unitCost || !goal || !mine}
          className="rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[11px] font-extrabold text-[var(--accent-ink)] disabled:opacity-40 cursor-pointer">Open the pool</button>
      </div>
      ...
      {out && (
        <div className="rounded-xl bg-[#EFF1F4] border border-[var(--brief-line)] p-2.5 space-y-1.5">
          ...
          <a href={out.waMe} target="_blank" rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-lg bg-[#4F46E5] px-3 py-1.5 text-[10.5px] font-extrabold text-[var(--accent-ink)] no-underline">
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
      <header className="space-y-1">
        <div className="flex items-center gap-2">
          <Send className="w-4 h-4 text-[#4F46E5]" aria-hidden="true" />
          <h1 className="text-lg font-extrabold text-[var(--brief-ink)]">WhatsApp shop</h1>
          {published && (
            <span className="px-2 py-0.5 rounded-full bg-[#4F46E5] text-[9px] font-extrabold uppercase tracking-[0.12em] text-[var(--accent-ink)]">Live</span>
          )}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-3">
          <div className="bg-[color:var(--color-paper)] border border-[var(--brief-line)] rounded-2xl p-4 space-y-3">
            <label className="block space-y-1">
              <span className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">Shop name</span>
              <input value={name} onChange={(e) => setName(e.target.value)} maxLength={40} placeholder="Mama Njeria Fresh"
                className="w-full rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-3 py-2 text-[13px] font-bold text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" />
            </label>
            <label className="block space-y-1">
              <span className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">One line about it</span>
              <input value={tagline} onChange={(e) => setTagline(e.target.value)} maxLength={60} placeholder="Fresh groceries, Kilimani"
                className="w-full rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-3 py-2 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" />
            </label>
            <label className="block space-y-1">
              <span className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">WhatsApp number customers order on</span>
              <input value={orderNumber} onChange={(e) => setOrderNumber(e.target.value)} maxLength={20} placeholder="+254 712 345 678" inputMode="tel"
                className="w-full rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-3 py-2 text-[13px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" />
            </label>
          </div>

          <div className="bg-[color:var(--color-paper)] border border-[var(--brief-line)] rounded-2xl p-4 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">Price list</span>
              <span className="text-[9px] text-[var(--ink-60)]">{items.length} item{items.length === 1 ? '' : 's'} · keep it forwardable</span>
            </div>
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-1.5">
                ...
                <input value={item.priceKes} onChange={(e) => setItems(items.map((it, i) => i === idx ? { ...it, priceKes: e.target.value.replace(/[^\d]/g, '') } : it))} placeholder="50" inputMode="numeric"
                  className="w-16 rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2 py-1.5 text-[12px] text-[var(--brief-ink)] outline-none focus:border-[#4F46E5]" aria-label={`price of item ${idx + 1} in shillings`} />
                <input value={item.note} onChange={(e) => setItems(items.map((it, i) => i === idx ? { ...it, note: e.target.value } : it))} placeholder="note (optional)" maxLength={40}
                  className="w-24 rounded-lg border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-2 py-1.5 text-[11px] text-[var(--ink-70)] outline-none focus:border-[#4F46E5]" aria-label={`note on item ${idx + 1}`} />
                <button type="button" onClick={() => setItems(items.filter((_, i) => i !== idx))} disabled={items.length === 1}
                  className="h-7 w-7 shrink-0 flex items-center justify-center rounded-lg text-[var(--ink-60)] hover:text-[#DC2626] disabled:opacity-30 cursor-pointer" aria-label={`remove item ${idx + 1}`}>
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
            <button type="button" onClick={() => setItems([...items, { name: '', priceKes: '', note: '' }])} disabled={items.length >= 40}
              className="w-full rounded-xl border border-dashed border-[var(--brief-line)] py-1.5 text-[11px] font-extrabold text-[#4F46E5] hover:border-[#4F46E5] disabled:opacity-40 cursor-pointer flex items-center justify-center gap-1">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => void save()} disabled={busy}
              className="rounded-xl bg-[#4F46E5] px-4 py-2 text-[12px] font-extrabold text-[var(--accent-ink)] disabled:opacity-40 cursor-pointer">Save</button>
            {published ? (
              <button type="button" onClick={() => void unpublish()} disabled={busy}
                className="rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-4 py-2 text-[12px] font-extrabold text-[var(--brief-ink)] disabled:opacity-40 cursor-pointer">Unpublish</button>
            ) : (
              <button type="button" onClick={() => void publish()} disabled={busy}
                className="rounded-xl border border-[#4F46E5] bg-[color:var(--color-paper)] px-4 py-2 text-[12px] font-extrabold text-[#4F46E5] disabled:opacity-40 cursor-pointer">Publish</button>
            )}
          </div>

          {error && (
            <div role="alert" className="rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-3 py-2 flex items-start gap-2">
              <ShieldCheck className="w-3.5 h-3.5 text-[#DC2626] shrink-0 mt-0.5" aria-hidden="true" />
              <div className="text-[11px] text-[var(--brief-ink)] leading-snug">
                {error}
                {!storeActive && /store service/i.test(error) && (
                  <>
                    {' '}
                    <button type="button" onClick={onOpenFees} className="font-extrabold text-[#4F46E5] underline cursor-pointer">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 px-1">
            <Store className="w-3.5 h-3.5 text-[#4F46E5]" aria-hidden="true" />
            <p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">What customers receive</p>
          </div>
          {view?.share ? (
            <>
              <div className="rounded-2xl bg-[#EFF1F4] border border-[var(--brief-line)] p-3">
                <div className="rounded-xl rounded-tl-sm bg-[color:var(--color-paper)] border border-[rgba(228, 218, 203, 0.60)] p-3 max-h-[380px] overflow-y-auto">
                  <pre className="whitespace-pre-wrap font-sans text-[11.5px] leading-relaxed text-[var(--brief-ink)] select-all">{view.share.text}</pre>
                </div>
                <p className="mt-1.5 px-1 text-[9px] text-[var(--ink-70)]">*stars* render bold and _underscores_ italic in WhatsApp — this is the exact text.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => void copyText()}
                  className="rounded-xl bg-[#4F46E5] px-3.5 py-2 text-[11px] font-extrabold text-[var(--accent-ink)] cursor-pointer flex items-center gap-1.5">
                  <Copy className="w-3.5 h-3.5" /> Copy message
                </button>
                <a href={view.share.waMe} target="_blank" rel="noreferrer"
                  className="rounded-xl border border-[var(--brief-line)] bg-[color:var(--color-paper)] px-3.5 py-2 text-[11px] font-extrabold text-[var(--brief-ink)] cursor-pointer flex items-center gap-1.5 no-underline">
                  <ExternalLink className="w-3.5 h-3.5" /> Open the wa.me link
                </a>
              </div>
              {!view.share.shareable && (
                <p className="px-1 text-[10px] text-[var(--ink-55)] leading-snug">
            ...
            <div className="rounded-2xl border border-dashed border-[var(--brief-line)] p-4 text-[11px] text-[var(--ink-55)]">
              Save the shop and the exact WhatsApp message appears here.
            </div>
          )}

          <div className="rounded-2xl bg-[color:var(--color-paper)] border border-[var(--brief-line)] p-3.5 space-y-1">
            <p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--ink-70)]">Store service</p>
            {view?.store.active ? (
              ...
              <>
                <p className="text-[11px] text-[var(--ink-70)] leading-snug">
                ...
                <button type="button" onClick={onOpenFees} className="text-[11px] font-extrabold text-[#4F46E5] underline cursor-pointer">
