...
const LOBBY_STYLE: Record<string, string> = {
  waiting_for_players: 'bg-[#D6CFE4] text-[#251045]/60',
  open: 'bg-[#5B2EA6] text-[#FFFFFF]',
  full: 'bg-[#D6CFE4] text-[#251045]',
  in_progress: 'bg-[#5B2EA6] text-[#FFFFFF]',
  completed: 'bg-[#FBFAFD] text-[#251045]',
  cancelled: 'bg-[#D6CFE4] text-[#251045]/40'
};
...
export function EplDesk({ meId, onToast }: { meId: string | null; onToast: (msg: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-extrabold text-[#251045]">EPL contest rooms</h2>
        ...
        {providerNote && (
          <p className="mt-1 text-[9px] leading-snug text-[#251045]/40">{providerNote}</p>
        )}
        {!meId && (
          <p className="mt-1 text-[10px] font-bold text-[#251045]/70">
      ...
      {/* create a room */}
      <div className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-3.5 space-y-2">
        <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#251045]">Open a room</p>
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className="min-w-[160px] flex-1 rounded-lg border border-[#D6CFE4] bg-[#F1EDF7] px-3 py-2 text-[12px] text-[#251045]"
          />
          <input
            type="datetime-local"
            value={kickoff}
            ...
            aria-label="kickoff"
            className="rounded-lg border border-[#D6CFE4] bg-[#F1EDF7] px-2.5 py-2 text-[12px] text-[#251045]"
          />
          <input
            value={budget}
            ...
            className="w-28 rounded-lg border border-[#D6CFE4] bg-[#F1EDF7] px-3 py-2 text-[12px] text-[#251045]"
          />
          <input
            value={minEntries}
        ...
        {formNote && <p className="text-[11px] text-[#251045]">{formNote}</p>}
      </div>

      {error && <p className="text-xs text-[#251045]">{error}</p>}
      {rooms === null && <p className="text-xs text-[#251045]/60">Loading rooms…</p>}
      {rooms !== null && rooms.length === 0 && !error && (
        <p className="text-xs text-[#251045]/60">No rooms yet. Open one — the waiting room is honest about being empty.</p>
      )}
      ...
      <div className="space-y-2">
        {(rooms ?? []).map((r) => (
          <div
            key={r.id}
            className={`rounded-2xl border bg-[#FBFAFD] p-3.5 space-y-2 ${selectedId === r.id ? 'border-[#6C3EC9]' : 'border-[#D6CFE4]'}`}
          >
            <button
              type="button"
            ...
            {selectedId === r.id && (
              <div className="space-y-2 border-t border-[#D6CFE4] pt-2">
                {r.mine && (
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      ...
                      disabled={busy}
                      className="rounded-lg border border-[#6C3EC9] px-3 py-1.5 text-[10px] font-extrabold text-[#251045] cursor-pointer disabled:opacity-40"
                    >
                      Import catalog pool
                    </button>
                    <button
                      ...
                      disabled={busy}
                      className="rounded-lg bg-[#5B2EA6] px-3 py-1.5 text-[10px] font-extrabold text-[#FFFFFF] cursor-pointer disabled:opacity-40"
                    >
                ...
                {pool !== null && pool.length > 0 && r.status !== 'cancelled' && (
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[10px] text-[#251045]/60">
                      <span>Pick 11 (1 GK · 3+ DEF · 2+ MID · 1+ FWD · max 3 per club)</span>
                      <span className={spend > (r.budgetKes ?? Infinity) ? 'font-extrabold text-[#251045]' : ''}>
                      ...
                      {pool.slice(0, 60).map((p) => {
                        ...
                        return (
                          <button
                            ...
                            className={`text-left rounded-lg border px-2 py-1.5 cursor-pointer ${picked ? 'border-[#6C3EC9] bg-[#5B2EA6] text-[#FFFFFF]' : 'border-[#D6CFE4] bg-[#F1EDF7] text-[#251045]'}`}
                          >
                            <p className="text-[10px] font-extrabold truncate">{p.name}</p>
                            <p className={`text-[9px] ${picked ? 'text-[#FFFFFF]/70' : 'text-[#251045]/40'}`}>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <select
                        ...
                        aria-label="captain"
                        className="rounded-lg border border-[#D6CFE4] bg-[#F1EDF7] px-2.5 py-2 text-[11px] text-[#251045]"
                      >
                    ...
                    {seatNote && <p className="text-[10px] text-[#251045] break-words">{seatNote}</p>}
                    <p className="text-[9px] leading-snug text-[#251045]/40">
                ...
                {standings !== null && standings.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-[9px] font-extrabold uppercase tracking-[0.14em] text-[#251045]/40">Standings</p>
                    {standings.map((s) => (
                      <div key={s.userId} className="flex items-center justify-between text-[10px] text-[#251045]">
