  ...
  return (
    <div className="space-y-4">
      ...
      {!openGroup && (
        <>
          <div>
            <h2 className="text-lg font-extrabold text-[#0D1117]">Your chats</h2>
          ...
          {visibleGroups.length === 0 && (
            <div className="border border-dashed border-[#E5E8EC] rounded-2xl p-8 text-center">
              <p className="text-xs text-[#0D1117]/60">No groups connected.</p>
            </div>
          )}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="shrink-0 px-3 py-1.5 rounded-xl bg-[#4F46E5] text-[#0D1117] font-extrabold text-[10px] cursor-pointer"
                  >
                    Open
                  </button>
                </div>
                ...
                <button
                  ...
                  className="text-[9px] text-[#0D1117]/60 underline underline-offset-2 cursor-pointer"
                >
      ...
      {openGroup && (
        <div>
          <button
            ...
            className="text-[10px] text-[#0D1117] cursor-pointer"
          >
            Back to your groups
          </button>

          <div className="flex items-center gap-2 mt-2">
            <h2 className="text-lg font-extrabold text-[#0D1117]">
              {openGroup.name}
            </h2>
      ...
      >
        <input
          ...
          className="flex-1 bg-[#FFFFFF] border border-[#E5E8EC] rounded-xl px-3 py-2.5 text-xs text-[#0D1117] placeholder:text-[#0D1117]/60 outline-none focus:border-[#E5E8EC]"
        />
        <button
          type="submit"
      ...
      <div className="flex flex-wrap gap-1.5">
        {['/brief', '/jobs', '/events', '/find solar', '/ask permit'].map((c) => (
          <button
            ...
            className="text-[10px] px-2 py-1 rounded-full bg-[#FFFFFF] border border-[#E5E8EC] text-[#0D1117] cursor-pointer"
          >
            {c}
          </button>
        ))}
      ...
      {commandResult && (
        <div className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4 space-y-3">
          ...
          {commandResult.brief && (
            <div className="space-y-2">
              ...
              {commandResult.brief.lines.map((line) => (
                ...
                >
                  <span className="text-[11px] text-[#0D1117]/60">{line.label}</span>
              ...
              {commandResult.brief.unanswered.length > 0 && (
                <div className="pt-2 border-t border-[#E5E8EC] space-y-1">
                  <p className="text-[10px] font-bold text-[#0D1117]">
                  ...
                  {commandResult.brief.unanswered.map((q) => (
                    <p key={q.id} className="text-[10px] text-[#0D1117]/60 leading-snug">
                      {q.originalText}
                    </p>
                  ))}
                </div>
          ...
          {commandResult.fromGroup.length > 0 && (
            <div className="space-y-2">
              ...
              {commandResult.fromGroup.slice(0, 6).map((entry) => (
                <div
                  key={entry.id}
                  className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-xl p-2.5"
                >
                  ...
                  <p className="text-[11px] text-[#0D1117] leading-snug mt-1">
                    {entry.originalText}
                  </p>

                  {entry.mediaKind && entry.mediaKind !== 'message' && (
                    <p className="text-[9px] text-[#0D1117]/60 mt-1">
                  ...
                  {entry.mediaExtractedText &&
                    entry.mediaAnalysisStatus === 'processed' && (
                      <p className="text-[10px] text-[#0D1117]/60 leading-snug mt-1 pl-2 border-l-2 border-[#E5E8EC]">
                        {entry.mediaExtractedText}
                      </p>
                    )}
                  ...
                  {entry.answers.map((a) => (
                    <p
                      key={a.messageId}
                      className="text-[10px] text-[#0D1117] leading-snug mt-1 pl-2 border-l-2 border-[#E5E8EC]"
                    >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {entry.entities.map((ent) => (
                      <span
                        key={ent.field}
                        className="text-[9px] text-[#0D1117]/60"
                      >
                        {ent.field}: {ent.value}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center gap-3 pt-2 mt-2 border-t border-[#E5E8EC]">
                    <button
                      onClick={() => handleSaveGroupEntry(entry)}
                      className="text-[9px] font-extrabold text-[#0D1117] cursor-pointer"
                    >
                      Save to My Layer
                    </button>
                    <button
                    ...
                    <span className="text-[9px] text-[#0D1117]/60 ml-auto">
                      From {openGroup.name}
                      {' - '}
            ...
            <div className="space-y-2">
              ...
              {commandResult.fromElsewhere.map((obj) => (
                <button
                  ...
                  className="w-full text-left bg-[#FFFFFF] border border-[#E5E8EC] hover:border-[#E5E8EC] rounded-xl p-2.5 cursor-pointer"
                >
                  <span className="text-[9px] text-[#0D1117]/60">
                    {getObjectTypeMeta(obj.type).label}
                  </span>
                  <p className="text-[11px] font-bold text-[#0D1117] mt-0.5">
                    {obj.title}
                  </p>
                </button>
              ))}
          ...
          {commandResult.emptyNote && (
            <p className="text-[11px] text-[#0D1117]/60">{commandResult.emptyNote}</p>
          )}
        </div>
      )}
      ...
      {unansweredQuestions.length > 0 && (
        <div className="space-y-2">
          ...
          {unansweredQuestions.map((q) => (
            <div
              key={q.id}
              className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-xl p-3"
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                {q.authorLabel && (
                  <span className="text-[9px] text-[#0D1117]/60">{q.authorLabel}</span>
                )}
                <span className="text-[9px] text-[#0D1117]/60">
                  {q.sentAt.slice(0, 10)}
                </span>
      ...
      <div className="border-t border-[#E5E8EC] pt-4 space-y-2">
        ...
        {[
          ...
          <div key={String(label)} className="flex items-baseline justify-between gap-3">
            ...
            <span className="text-[10px] text-[#0D1117]/60">{value}</span>
          </div>
        ))}
      </div>
      </>
