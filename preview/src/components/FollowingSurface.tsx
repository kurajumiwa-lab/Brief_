    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 border-b border-[#D6CFE4] px-4 py-3">
          ...
          <div className="flex items-center gap-1 rounded-full border border-[#D6CFE4] bg-white p-1">
            <button
              type="button"
              onClick={() => setTab('following')}
              className={`rounded-full px-3 py-1 text-[11px] font-bold transition-colors ${tab === 'following' ? 'bg-[#5B2EA6] text-white' : 'text-[#251045] hover:bg-[#F1EDF7]'}`}
            >
              Following
            </button>
            <button
              ...
              className={`rounded-full px-3 py-1 text-[11px] font-bold transition-colors ${tab === 'manage' ? 'bg-[#5B2EA6] text-white' : 'text-[#251045] hover:bg-[#F1EDF7]'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto pb-safe">
          {!authed && (
            <div className="flex flex-col items-center gap-3 px-6 py-20 text-center">
              <Users className="h-8 w-8 text-[#5B2EA6]" />
              ...
              <button
                ...
                className="rounded-full bg-[#5B2EA6] px-5 py-2.5 text-[13px] font-bold text-white"
              >
                Sign in
              </button>
            </div>
            ...
            <div className="space-y-5 px-4 pb-10 pt-4 sm:px-5">
              {(!feed || feed.length === 0) && (
                <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-[#D6CFE4] px-6 py-12 text-center">
                  <Plus className="h-6 w-6 text-[rgba(37,16,69,0.4)]" />
                  <p className="text-[13px] font-semibold text-[#251045]">Nothing followed yet</p>
              ...
              {feed?.map((section) => (
                <section key={section.entityId} aria-label={section.name}>
                  ...
                  >
                    <span className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#251045] group-hover:text-[#5B2EA6]">
                      {section.name}
                    </span>
                    ...
                    {section.location?.area && (
                      <span className="flex items-center gap-0.5 text-[10px] font-semibold text-[rgba(37,16,69,0.62)]">
                    ...
                    {section.objects.map((o) => {
                      ...
                      return (
                        <button
                          ...
                          className="group flex items-stretch gap-2.5 rounded-2xl border border-[#D6CFE4] bg-white p-2 text-left shadow-sm transition-transform hover:-translate-y-0.5 hover:border-[#6C3EC9]"
                        >
                          {o.imageUrl ? (
                          ...
                          <div className="min-w-0 py-0.5">
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              {line && <span className="text-[9px] font-semibold text-[rgba(37,16,69,0.62)]">{line}</span>}
                            </div>
          ...
          {authed && !loading && tab === 'manage' && (
            <div className="px-4 pb-10 pt-4 sm:px-5">
              {kindOrder.length === 0 && (
                <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-[#D6CFE4] px-6 py-12 text-center">
                  <Users className="h-6 w-6 text-[rgba(37,16,69,0.4)]" />
                  <p className="text-[13px] font-semibold text-[#251045]">You're not following anything yet</p>
                  <p className="max-w-xs text-[12px] leading-relaxed text-[rgba(37,16,69,0.62)]">
              ...
              {kindOrder.map((kind) => {
                ...
                return (
                  <section key={kind} aria-label={KIND_LABELS[kind]} className="mb-5">
                    <h3 className="mb-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#251045]">
                      {KIND_LABELS[kind]} <span className="text-[rgba(37,16,69,0.45)]">· {items.length}</span>
                    </h3>
                    <div className="grid gap-2">
                      {items.map((f) => (
                        <div
                          key={f.id}
                          className="flex items-center gap-3 rounded-2xl border border-[#D6CFE4] bg-white p-2.5 shadow-sm"
                        >
                          {f.imageUrl ? (
                            <img src={f.imageUrl} alt="" aria-hidden="true" className="h-10 w-10 rounded-xl object-cover" />
                          ) : (
                            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#5B2EA6] to-[#3A2169] text-[#FFFFFF]">
                          ...
                          >
                            <p className="truncate text-[13px] font-bold text-[#251045] hover:text-[#5B2EA6]">{f.name}</p>
                            <p className="truncate text-[10px] font-semibold text-[rgba(37,16,69,0.62)]">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 rounded-full border border-[#D6CFE4] px-2.5 py-1.5 text-[10px] font-bold text-[#251045] transition-colors hover:border-[#C0392B] hover:bg-[#FDF0EE] hover:text-[#C0392B]"
                          >
              ...
              {kindOrder.length > 0 && (
                <p className="flex items-center gap-1.5 text-[10px] font-semibold text-[rgba(37,16,69,0.5)]">
