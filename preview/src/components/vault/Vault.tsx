  ...
  return (
    <button
      ...
      className={`shrink-0 px-2.5 py-1 rounded-full text-[10px] font-extrabold border cursor-pointer transition ${
        active ? 'bg-[#00FF42] text-[#09150E] border-[#00FF42]' : 'bg-[#102117] text-[#8DCF74] border-[#235F45]'
      }`}
    >
      {children}
    </button>
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <div className={`bg-[#0B1A12] border border-[#1E3A2A] rounded-xl ${className}`}>{children}</div>;
}
    ...
    <div className="space-y-3">
      ...
      {days.map((d) => (
        <div key={d.day} className="space-y-1">
          <p className="text-[9px] font-mono tracking-[0.2em] text-[#5C6B52] mt-3 mb-1">{d.day}</p>
          {d.items.map((f) => (
            <div key={f.id} className="flex items-start gap-2.5 py-1.5 border-b border-[#122A1B] last:border-0">
              <span
                className="mt-1.5 shrink-0 w-2 h-2 rounded-full"
                style={{ background: CATEGORY_TONE[f.category] ?? '#86935C' }}
              />
              <div className="min-w-0">
                <p className="text-[11px] leading-snug text-[#E2ECE5]">{f.narrative}</p>
      ...
      {page.nextCursor !== null && (
        <button
          ...
          className="w-full py-2 rounded-lg bg-[#102117] text-[#8DCF74] text-[10px] font-extrabold border border-[#235F45] cursor-pointer"
        >
          Load earlier
        </button>
      )}
...
function HostPanel({ vault, onChanged }: { vault: VaultType; onChanged: () => void }) {
  ...
  return (
    <Card className="p-3 space-y-2">
      <p className="text-[11px] font-extrabold text-[#E8D48B]">Host</p>

      <div className="flex gap-1.5">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name"
          className="flex-1 min-w-0 bg-[#09150E] border border-[#235F45] rounded-lg px-2.5 py-1.5 text-xs text-[#E2ECE5] placeholder:text-[#5C6B52] outline-none focus:border-[#00FF42]" />
        <select value={role} onChange={(e) => setRole(e.target.value)}
          className="bg-[#09150E] border border-[#235F45] rounded-lg px-2 py-1.5 text-xs text-[#E2ECE5] outline-none">
          <option value="guest">Guest</option>
          <option value="vendor">Vendor</option>
          <option value="admin">Admin</option>
        </select>
        <button onClick={addParticipant} disabled={busy || !name.trim()}
          className="px-2.5 py-1.5 rounded-lg bg-[#00FF42] text-[#09150E] text-[10px] font-extrabold cursor-pointer disabled:opacity-40">
          Add
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        <button onClick={makeHandoff} disabled={busy}
          className="px-2.5 py-1.5 rounded-lg bg-[#172D20] text-[#8DCF74] text-[10px] font-extrabold border border-[#235F45] cursor-pointer disabled:opacity-40">
          Continue elsewhere
        </button>
        <button onClick={async () => { await briefApi.closeVault(vault.id); onChanged(); }}
          className="px-2.5 py-1.5 rounded-lg bg-[#2A1515] text-[#E57F7F] text-[10px] font-extrabold border border-[#3A1A1A] cursor-pointer">
          Close vault
        </button>
      </div>

      {handoff && (
        <div className="bg-[#09150E] border border-[#1E3A2A] rounded-lg p-2 space-y-1">
          <p className="text-[9px] font-mono text-[#5C6B52]">Handoff link (single-use):</p>
          <p className="text-[10px] text-[#8DCF74] break-all select-all">{handoff}</p>
        </div>
      )}
      {note && <p className="text-[10px] text-[#C9A227]">{note}</p>}
    </Card>
  );
}
...
function Requests({ vault, onChanged }: { vault: VaultType; onChanged: () => void }) {
  ...
  return (
    <Card className="p-3 space-y-2">
      <p className="text-[11px] font-extrabold text-[#E8D48B]">Requests</p>
      {requests.map((r: VaultRequest) => (
        <div key={r.id} className="bg-[#09150E] border border-[#1E3A2A] rounded-lg p-2.5 flex items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="text-xs text-[#E2ECE5] truncate">{r.description}</p>
            <p className="text-[9px] font-mono uppercase text-[#5C6B52] mt-0.5">{r.status}</p>
          </div>
          {vault.role === 'vendor' && (r.status === 'routed' || r.status === 'open') && (
            <button onClick={async () => { await briefApi.acceptVaultRequest(vault.id, r.id); onChanged(); }}
              className="shrink-0 px-2.5 py-1.5 rounded-lg bg-[#00FF42] text-[#09150E] text-[10px] font-extrabold cursor-pointer">
              Accept
            </button>
          )}
        </div>
...
function VaultDetail({ vault, onBack, onChanged }: { vault: VaultType; onBack: () => void; onChanged: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <button onClick={onBack} className="text-[10px] font-extrabold text-[#8DCF74] cursor-pointer">← Vaults</button>

      <div>
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-lg font-extrabold text-[#E2ECE5] leading-tight">{vault.title}</h2>
          <span className="shrink-0 text-[9px] font-mono uppercase px-2 py-0.5 rounded-full bg-[#172D20] text-[#8DCF74]">{vault.status}</span>
        </div>
      ...
      {vault.role !== 'vendor' && (
        <Card className="p-3 space-y-2">
          <input value={ask} onChange={(e) => setAsk(e.target.value)} placeholder="Ask a question…"
            className="w-full bg-[#09150E] border border-[#235F45] rounded-lg px-3 py-2 text-xs text-[#E2ECE5] placeholder:text-[#5C6B52] outline-none focus:border-[#00FF42]" />
...
function Stat({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="bg-[#0B1A12] border border-[#1E3A2A] rounded-lg px-3 py-2">
      <p className="text-[9px] font-mono uppercase tracking-wider text-[#5C6B52]">{label}</p>
      <p className={`text-sm font-extrabold ${accent ? 'text-[#00FF42]' : 'text-[#E2ECE5]'}`}>{value}</p>
    </div>
  );
}
...
function VaultHome({ onOpen, refreshKey }: { onOpen: (v: VaultType) => void; refreshKey: number }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-lg font-extrabold text-[#E2ECE5]">The Vault</h2>
        <button onClick={() => setCreating((c) => !c)}
          className="px-3 py-1.5 rounded-full bg-[#00FF42] text-[#09150E] text-[10px] font-extrabold cursor-pointer">
          + New vault
        </button>
      </div>
        ...
        <Card className="p-3 space-y-2">
          ...
          <div className="flex gap-1.5">
            <select value={type} onChange={(e) => setType(e.target.value)}
              className="flex-1 bg-[#09150E] border border-[#235F45] rounded-lg px-2 py-1.5 text-xs text-[#E2ECE5] outline-none">
          ...
          <button onClick={create} disabled={!title.trim()}
            className="w-full py-2 rounded-lg bg-[#00FF42] text-[#09150E] text-[10px] font-extrabold cursor-pointer disabled:opacity-40">
            Create
          </button>
        </Card>
      )}
      ...
      <div className="relative">
        <input value={q} onChange={(e) => void search(e.target.value)} placeholder="Search people, requests, payments…"
          className="w-full bg-[#0B1A12] border border-[#1E3A2A] rounded-xl px-3 py-2.5 text-xs text-[#E2ECE5] placeholder:text-[#5C6B52] outline-none focus:border-[#00FF42]" />
      </div>
      {results.length > 0 && (
        <Card className="p-3 space-y-1.5">
          {results.map((r) => (
            <button key={r.vaultId} onClick={() => { const v = vaults.find((x) => x.id === r.vaultId); if (v) onOpen(v); }}
              className="w-full text-left bg-[#09150E] border border-[#1E3A2A] rounded-lg p-2.5 cursor-pointer">
              <p className="text-xs font-extrabold text-[#E2ECE5]">{r.title}</p>
              <p className="text-[9px] text-[#5C6B52] truncate">{r.matches.map((m) => m.snippet).join(' · ')}</p>
            </button>
          ))}
        </Card>
      )}
      ...
      {resolution.length > 0 && (
        <Card className="p-3 space-y-1.5">
          <p className="text-[11px] font-extrabold text-[#C9A227]">Needs attention</p>
          {resolution.map((r, i) => (
            <div key={i} className="flex items-center justify-between gap-2">
              <p className="text-[10px] text-[#E2ECE5] truncate">{r.vaultTitle} · {r.description ?? r.kind}</p>
            </div>
          ))}
        </Card>
      )}

      {status === 'loading' && <p className="text-xs text-[#86935C]">Loading…</p>}
      {status === 'error' && <p className="text-xs text-[#C4553E]">Could not load vaults.</p>}
      {status === 'ready' && vaults.length === 0 && (
        <p className="text-xs text-[#86935C]">No vaults yet. A vault wraps a real activity — a gathering, a market, a deal.</p>
      )}

      {vaults.filter((v) => v.status !== 'closed' && v.status !== 'archived').map((v) => (
        <button key={v.id} onClick={() => onOpen(v)} className="w-full text-left">
          <Card className="p-3 hover:border-[#235F45] transition">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-sm font-extrabold text-[#E2ECE5] truncate">{v.title}</p>
                <p className="text-[9px] font-mono uppercase tracking-wider text-[#5C6B52] mt-0.5">{v.type} · {v.role}</p>
              </div>
              <span className={`shrink-0 text-[9px] font-mono uppercase px-2 py-0.5 rounded-full ${
                v.status === 'settled' ? 'bg-[#172D20] text-[#00FF42]' : 'bg-[#1A2A3A] text-[#7FB2E5]'
              }`}>{v.status}</span>
            </div>
            <div className="flex gap-3 mt-2 text-[10px] text-[#86935C]">
              <span>{v.metrics.participantCount} people</span>
              {v.metrics.requestCount > 0 && <span>{v.metrics.requestCount} requests</span>}
              {v.metrics.pendingKes > 0 && <span className="text-[#E8D48B]">{money(v.metrics.pendingKes)} pending</span>}
            </div>
          </Card>
        </button>
      ))}
