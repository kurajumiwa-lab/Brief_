  ...
  return (
    <section aria-label="Your account" className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-3.5">
      <div className="flex items-center gap-3">
        ...
        >
          <span className="h-10 w-10 rounded-xl bg-[#4F46E5] text-[#0D1117] flex items-center justify-center text-[13px] font-black shrink-0" aria-hidden="true">
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="shrink-0 rounded-lg px-2 py-1 text-[11px] font-extrabold text-[#4F46E5] hover:bg-[#F0F2F5] cursor-pointer"
        >
          {open ? 'Close' : 'View'} →
        </button>
      </div>
        ...
        <div className="mt-3 pt-3 border-t border-[#E5E8EC] space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 h-9 rounded-xl bg-[#4F46E5] text-[#0D1117] text-[11.5px] font-extrabold flex items-center justify-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="grid grid-cols-2 gap-2">
        {EXPLORE.map(({ label, detail, Icon, target }) => (
          <button
            ...
            className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-3 text-left hover:border-[#4F46E5] transition-colors cursor-pointer"
          >
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-2.5 text-center transition-colors cursor-pointer ${
                selected
                  ? 'border-[#4F46E5] bg-[#FFFFFF] shadow-sm'
                  : 'border-[#E5E8EC] bg-[#FFFFFF] hover:border-[#4F46E5]'
              }`}
            >
              <span className="block text-[20px] leading-none" aria-hidden="true">{region.flag}</span>
              <span className={`mt-1 block truncate text-[10px] font-extrabold ${selected ? 'text-[#4F46E5]' : 'text-[#0D1117]'}`}>
                {region.label}
              </span>
            </button>
          );
      ...
      <header className="shrink-0 flex items-start justify-between gap-3 px-4 pt-5 pb-3 sm:max-w-3xl sm:w-full sm:mx-auto">
        <div className="flex items-center gap-2.5">
          {/* §13 — the Brief mark: a small, ownable brand mark, not another
              product's logo. */}
          <span className="h-8 w-8 shrink-0 rounded-lg bg-[#4F46E5] text-[#0D1117] flex items-center justify-center text-[14px] font-black" aria-hidden="true">B</span>
          <div>
        ...
        <button
          ...
          className="h-10 w-10 shrink-0 flex items-center justify-center rounded-full bg-[#FFFFFF] border border-[#E5E8EC] text-[#0D1117] text-[20px] font-light hover:border-[#4F46E5] transition-colors cursor-pointer"
        >
          ×
        </button>
      </header>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-[84px] md:pb-8 space-y-4 sm:max-w-3xl sm:w-full sm:mx-auto">
        ...
        {canOperate && (
          <button
            ...
            className="w-full bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl px-3.5 py-2.5 flex items-center gap-3 hover:border-[#4F46E5] transition-colors cursor-pointer"
          >
            <span className="h-7 w-7 rounded-lg bg-[#F0F2F5] flex items-center justify-center shrink-0">
