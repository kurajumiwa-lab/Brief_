...
const STATUS_COLOR: Record<string, string> = {
  draft: '#5A6472', review: '#5A6472', approved: '#0D1117', published: '#0D1117',
  scheduled: '#4B5563', expired: '#5A6472', archived: '#E5E8EC'
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-extrabold text-[#0D1117]">Editorial Studio</h2>
        ...
        <button
          ...
          className="rounded-full bg-[#4F46E5] px-4 py-2 text-[12px] font-bold text-[#0D1117] cursor-pointer"
        >
          New story
        </button>
      </div>
        ...
        <div className="space-y-2">
          {articles.length === 0 && (
            <div className="rounded-2xl border border-[#E5E8EC] bg-[#FFFFFF] p-4">
              <p className="text-[13px] font-bold text-[#0D1117]">No stories yet.</p>
              <p className="mt-1 text-[11px] text-[#0D1117]/60">
          ...
          {articles.map((a) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 rounded-xl border border-[#E5E8EC] bg-[#FFFFFF] p-3">
                {/* design + hero thumbnail: what the story looks like */}
                <div
                  className="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-[#E5E8EC]"
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-[#0D1117] truncate">{a.title}</p>
                  <p className="text-[10px] text-[#0D1117]/60">
                ...
                <div className="flex shrink-0 gap-1">
                  <button onClick={() => setEditing(a)} className="text-[10px] font-bold text-[#0D1117] cursor-pointer">Design</button>
                  {a.status !== 'published' ? (
                    <button onClick={() => void transition(a.id, 'publish')} className="text-[10px] font-bold text-[#0D1117] cursor-pointer">Publish</button>
                  ) : (
                    <button onClick={() => void transition(a.id, 'unpublish')} className="text-[10px] font-bold text-[#0D1117] cursor-pointer">Unpublish</button>
                  )}
                  {a.status !== 'archived' && (
                    <button onClick={() => void transition(a.id, 'archive')} className="text-[10px] font-bold text-[#0D1117]/60 cursor-pointer">Archive</button>
                  )}
                </div>
              </div>
            );
