...
import { Rocket, Smartphone, X } from 'lucide-react';
...
export const WairoFloatingWidget: React.FC = () => {
  ...
  return (
    <aside aria-label="Wairo Mini App Companion" data-wairo-widget="true">
      ...
      {!isOpen && (
        <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 40 }}>
          <button
            ...
            className="px-4 py-2.5 rounded-full bg-[#0B1B2A] hover:bg-[#173247] border-2 border-[#00BFEF] text-white shadow-2xl flex items-center space-x-3 transition-all cursor-pointer group"
          >
            <div className="w-8 h-8 rounded-full bg-[#F58220] flex items-center justify-center text-white font-bold shadow-md shadow-[#F58220]/30">
              <Rocket className="w-4 h-4" />
            </div>
            <div className="text-left pr-1">
              <div className="flex items-center space-x-1.5">
                <span className="text-xs font-bold text-white">Wairo Delivery</span>
                <span className="w-2 h-2 rounded-full bg-[#00BFEF] animate-ping"></span>
              </div>
              <span className="text-[10px] text-[#00BFEF] font-mono font-semibold">
                {activeDelivery.status} • {selectedLocation.name} ({selectedLocation.etaMins}m)
              </span>
            </div>
          </button>
        </div>
      ...
      {/* Floating Expanded Mini App Drawer */}
      {isOpen && !isFullPhoneMode && (
        <div style={{ position: 'fixed', bottom: '16px', right: '16px', zIndex: 45 }} className="w-[95vw] sm:w-[400px] h-[660px] max-h-[88vh] shadow-2xl">
      ...
      {/* Full Modal Smartphone Mode */}
      {isOpen && isFullPhoneMode && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50 }} className="bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative w-[380px] sm:w-[412px] h-[780px] sm:h-[820px] bg-[#0B1B2A] rounded-[50px] p-3 shadow-2xl border-4 border-[#173247] ring-8 ring-black/40 flex flex-col">
            ...
            >
              <span>Dock to Floating Mini App</span>
              <X className="w-3.5 h-3.5" />
            </button>
