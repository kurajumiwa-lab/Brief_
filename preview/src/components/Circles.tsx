  ...
  if (!openId) {
    return (
      <section className="space-y-4">
        <div>
          <h2 className="text-lg font-extrabold text-[#F3F1E7]">Circles</h2>
  ...
  return (
    <section className="space-y-4">
      ...
      {detail.status === 'loading' && (
        <p className="text-xs text-[#8A93A6] mt-2">Loading...</p>
      )}

      {detail.status === 'error' && (
        <p className="text-[11px] text-[#E8A33D] mt-2">
      ...
      {detail.status === 'ready' && open && (
        <>
          <div className="mt-2">
            <h2 className="text-lg font-extrabold text-[#F3F1E7]">{open.name}</h2>
            <p className="text-[9px] text-[#4B5162] mt-0.5">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-[10px] font-extrabold cursor-pointer ${
                  section === s.id
                    ...
                    : 'bg-[#10141C] border border-[#232A38] text-[#8A93A6]'
                }`}
              >
                {s.label}
              </button>
          ...
          {notice && (
            <div className="border border-[#10141C] bg-[#10141C] rounded-xl px-3 py-2">
              <p className="text-[10px] text-[#E8A33D] leading-snug">{notice}</p>
            </div>
          )}
          ...
          {section === 'overview' && (
            <div className="space-y-4">
              {open.goal && (
                <div>
                  <h3 className="text-[11px] font-extrabold text-[#4B5162] mb-1">
                    Purpose
                  </h3>
                  <p className="text-[11px] text-[#43D17A] leading-snug">{open.goal}</p>
                </div>
              )}
              ...
              <div>
                ...
                {detail.blocks.filter((b) => b.type !== 'task' && b.type !== 'vote')
                  .length === 0 ? (
                  <p className="text-xs text-[#8A93A6]">Nothing posted yet.</p>
                ) : (
                  <div className="space-y-2">
                    {detail.blocks
                      ...
                      .map((block) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-[#F3F1E7] mt-1">{block.content}</p>
