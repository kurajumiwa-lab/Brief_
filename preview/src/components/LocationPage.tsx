  ...
  return (
    ...
    >
      {image ? (
        ...
        <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center gap-1 rounded-xl bg-gradient-to-br from-[#EFF1F4] to-[#E5E8EC] text-[#5A6472]">
      ...
      <div className="min-w-0 py-0.5">
        ...
        {(object.area || object.county || object.locationName) && (
          <p className="mt-0.5 flex items-center gap-1 truncate text-[9px] font-semibold text-[#0D1117]/55">
...
function Section({ title, icon, objects, onOpenObject }: {
  ...
  return (
    <section aria-label={title} className="mb-5">
      <h3 className="mb-2 flex items-center gap-1.5 text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#0D1117]">
        {icon}{title}
        <span className="text-[#0D1117]/60">· {objects.length}</span>
      </h3>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-2 border-b border-[#E5E8EC] px-4 py-3">
          <button type="button" onClick={onClose} className="flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[12px] font-bold text-[#0D1117] transition-colors hover:bg-[#F0F2F5]">
            <ArrowLeft className="h-4 w-4" /> Back
          </button>
          <span className="truncate text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#0D1117]/70">
            Explore
          </span>
          <button type="button" onClick={onClose} aria-label="Close" className="rounded-full p-2 text-[#0D1117] transition-colors hover:bg-[#F0F2F5]">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pb-safe">
          {missing && (
            <div className="flex flex-col items-center gap-3 px-6 py-24 text-center">
              ...
              <p className="max-w-xs text-[12px] leading-relaxed text-[#0D1117]/60">
            ...
            <div className="px-4 pb-10 pt-5 sm:px-5">
              ...
              <div className="mb-1 flex flex-wrap items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-[#FF5A1F]">
                ...
                {page.location.county && page.location.kind !== 'county' && (
                  <button
                    ...
                    className="rounded-full bg-[#F0F2F5] px-2 py-0.5 hover:bg-[#EFF1F4]"
                ...
                {page.location.kind === 'county' && Array.isArray(page.location.areas) && page.location.areas.length > 0 && (
                  <span className="rounded-full bg-[#F0F2F5] px-2 py-0.5 text-[#0D1117]/70">
                    {page.location.areas.length} areas
                  </span>
                )}
              </div>
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="text-[26px] font-extrabold leading-tight text-[#0D1117]">{page.location.name}</h2>
                {authed ? (
                  <button
                    type="button"
              ...
              <div className="mb-5 grid grid-cols-4 gap-2">
                {([
                  ...
                  <div key={label} className="rounded-2xl border border-[#E5E8EC] bg-[#FFFFFF] px-2 py-2.5 text-center shadow-sm">
                    <div className="flex items-center justify-center gap-1 text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#0D1117]/70">
                      {icon}{label}
                    </div>
                    <p className="mt-1 text-[22px] font-extrabold leading-none text-[#0D1117]">{count}</p>
                  </div>
                ))}
              </div>
              ...
              {page.nearby.available === false && (
                <p className="mb-5 flex items-center gap-1.5 rounded-xl bg-[#F0F2F5] px-3 py-2 text-[10px] font-semibold text-[#0D1117]/60">
              ...
              <p className="mt-6 flex items-center gap-1.5 text-[9px] font-semibold text-[#0D1117]/60">
