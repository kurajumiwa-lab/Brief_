  ...
  if (votes.length === 0) {
    return (
      <div>
        ...
        <p className="text-xs text-[#111111]/60">No votes in this circle.</p>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <p className="text-xs font-extrabold text-[#111111] min-w-0">
            {vote.content}
          </p>
          {isClosed && (
            ...
            return (
              <div key={r.option} className="space-y-1">
                <div className="flex items-center justify-between gap-3">
                  <span
                    className={`text-[11px] ${
                      isLeader ? 'text-[#111111] font-extrabold' : 'text-[#111111]/60'
                    }`}
                  >
                    {r.option}
                  </span>
                ...
                <div className="h-1 bg-[#FAFAFA] rounded-full overflow-hidden">
                  <div
                    className={`h-full ${isLeader ? 'bg-[#111111]' : 'bg-[#E5E7EB]'}`}
                    style={{ width: `${r.pct ?? 0}%` }}
                  />
                </div>
              </div>
        ...
        {!isClosed && myRole === 'observer' && (
          <p className="text-[10px] text-[#111111]/40">Observers cannot vote.</p>
        )}
        ...
        {!isClosed && isCoordinator && (
          <button
            ...
            disabled={busy}
            className="text-[10px] font-extrabold text-[#111111] cursor-pointer disabled:opacity-50"
          >
            Close this vote
          </button>
        )}
