  ...
  return (
    <div>
      ...
      {members.length === 0 && !canGovern && (
        <p className="text-xs text-[#0D1117]/60">No members yet.</p>
      )}
      {members.length === 0 && canGovern && (
      ...
      <div className="space-y-2">
        ...
        {canGovern && (
          <form
            ...
            className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-3 space-y-2"
          >
            <p className="text-[10px] font-extrabold text-[#0D1117]/60">
              Invite a member
            </p>
            <div className="flex items-center gap-2">
              <input
                ...
                className="min-w-0 flex-1 px-2.5 py-1.5 rounded-xl border border-[#E5E8EC] text-[10px] text-[#0D1117]"
              />
              <select
                value={inviteRole}
                onChange={(e) => setInviteRole(e.target.value as Member['role'])}
                aria-label="invite role"
                className="px-2 py-1.5 rounded-xl border border-[#E5E8EC] text-[10px] text-[#0D1117]"
              >
                {GOV_ROLES.map((r) => (
        ...
        {members.map((member) => {
            ...
            return (
              ...
              >
                ...
                {canGovern && member.userId !== currentUserId && (
                  <div className="flex items-center gap-2 pt-1">
                    <select
                      ...
                      className="px-2 py-1 rounded-xl border border-[#E5E8EC] text-[9px] text-[#0D1117] cursor-pointer disabled:opacity-50"
                    >
                      {GOV_ROLES.map((r) => (
                        <option key={r} value={r}>
                          {ROLE_LABEL[r] ?? r}
                    ...
                    {confirmRemove === member.userId ? (
                      <span className="flex items-center gap-1.5 ml-auto">
                        <button
                          ...
                          disabled={busyUserId === member.userId}
                          className="px-2.5 py-1 rounded-xl bg-[#4F46E5] text-[#0D1117] font-extrabold text-[9px] cursor-pointer disabled:opacity-50"
                        >
                          Confirm remove
                        </button>
                        <button
                          onClick={() => setConfirmRemove(null)}
                          className="px-2.5 py-1 rounded-xl border border-[#E5E8EC] font-bold text-[9px] text-[#0D1117]/60 cursor-pointer"
                        >
                          Keep
                        </button>
                      </span>
                      ...
                      <button
                        ...
                        disabled={busyUserId === member.userId}
                        className="ml-auto px-2.5 py-1 rounded-xl border border-[#E5E8EC] font-bold text-[9px] text-[#0D1117]/60 cursor-pointer disabled:opacity-50"
                      >
                        Remove
                      </button>
                    )}
                ...
                {member.trust.evidence.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {member.trust.evidence.map((e) => (
                      <span
                        key={e.kind}
                        className="text-[9px] px-2 py-0.5 rounded-full bg-[#FFFFFF] text-[#0D1117]"
                      >
                        {e.label}
                      </span>
                    ))}
                ...
                {open && (
                  <div className="pt-1 space-y-2 border-t border-[#E5E8EC]">
                    {/* Plain factual counts from the server. */}
                    {member.trust.facts.length > 0 && (
                      <ul className="space-y-0.5 mt-2">
                        {member.trust.facts.map((f) => (
                          <li key={f.kind} className="text-[10px] text-[#0D1117]/60">
                            {f.label}
                          </li>
                        ))}
                      </ul>
                    ...
                    {ev === 'loading' && (
                      <p className="text-[10px] text-[#0D1117]/60">Loading evidence...</p>
                    )}
                        ...
                        {ev.summary.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {ev.summary.map((s) => (
                              <span
                                key={s.kind}
                                className="text-[9px] px-2 py-0.5 rounded-full bg-[#F0F2F5] border border-[#E5E8EC] text-[#0D1117]/60"
                              >
                                {s.label}
                              </span>
                            ))}
                        ...
                        {ev.evidence.length === 0 ? (
                          ...
                          <ul className="space-y-1">
                            {ev.evidence.slice(0, 10).map((item) => (
                              <li
                                key={item.signalId}
                                className="flex items-center gap-2 text-[10px] text-[#0D1117]/60"
                              >
                                <span className="min-w-0 truncate">{item.label}</span>
                                <span className="text-[9px] text-[#0D1117]/60 ml-auto shrink-0">
                                  {item.at.slice(0, 10)}
                                </span>
                              </li>
                            ))}
