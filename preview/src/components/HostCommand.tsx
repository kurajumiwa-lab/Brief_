...
export function HostCommand() {
  ...
  if (state.status === 'loading' && !d) {
    return <p className="text-xs text-[var(--ink-60)]">Loading…</p>;
  }
  if (state.status === 'error') {
    return <p className="text-xs text-[var(--brief-ink)]">{state.error}</p>;
  }
  if (!d) return null;
    ...
    <div className="space-y-3">
      ...
      <Section title="Now">
        {d.now.length === 0 && d.upcoming.length === 0 ? (
          <p className="text-xs text-[var(--ink-60)]">Nothing needs you right now.</p>
        ) : (
          <>
