  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] text-[#F7F7F8]/60">{rooms.length} room{rooms.length === 1 ? '' : 's'} open</span>
      </div>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-[#222630] bg-[#12151A] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]"
        />
        <input
          value={mode}
          onChange={(e) => setMode(e.target.value)}
          placeholder="Mode (e.g. S&D)"
          className="hidden sm:block w-32 rounded-lg border border-[#222630] bg-[#12151A] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]"
        />
        <button
          onClick={() => void host()}
          disabled={busy || code.length < 4}
          className="flex items-center gap-1 rounded-lg bg-[#FF5A1F] px-3 py-2 text-[12px] font-bold text-[#0D0F12] disabled:opacity-40 cursor-pointer"
        >
      ...
      {status === 'loading' ? (
        <p className="text-[11px] text-[#F7F7F8]/60">Loading rooms…</p>
      ) : rooms.length === 0 ? (
        <p className="text-[11px] text-[#F7F7F8]/60">No rooms open. Host one and drop the code.</p>
      ) : (
        <div className="space-y-2">
          {rooms.map((r) => (
            <div key={r.id} className="flex items-center gap-3 rounded-xl border border-[#222630] bg-[#12151A] p-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[16px] font-bold text-[#F7F7F8]">{r.code}</span>
                  {r.mode && <span className="text-[10px] text-[#F7F7F8]/60">· {r.mode}</span>}
                  {r.hostTrust?.verified && (
                    <span className="rounded-full bg-[#FF5A1F]/10 px-2 py-0.5 text-[9px] font-bold text-[#F7F7F8]">✓ {r.hostTrust.label}</span>
                  )}
                </div>
                <div className="mt-0.5 flex items-center gap-1 text-[10px] text-[#F7F7F8]/60">
                ...
                {/* Host trust is a tally of real vouches, never a score. The
                    ...
                    from the server. */}
                <div className="mt-1 flex items-center gap-1.5 text-[10px] text-[#F7F7F8]/60">
                  <span>host vouches:</span>
                  <button
                    onClick={() => void vouch(r.hostId, true)}
                    disabled={vouchBusy === r.hostId}
                    className="rounded-md border border-[#222630] px-1.5 py-0.5 text-[10px] font-bold text-[#F7F7F8] cursor-pointer disabled:opacity-50"
                  ...
                  <button
                    ...
                    disabled={vouchBusy === r.hostId}
                    className="rounded-md border border-[#222630] px-1.5 py-0.5 text-[10px] font-bold text-[#F7F7F8]/60 cursor-pointer disabled:opacity-50"
              ...
              <button
                ...
                className="rounded-lg border border-[#222630] px-3 py-1.5 text-[11px] font-bold text-[#F7F7F8] hover:border-[#22E6E0] cursor-pointer"
              >
                Join
              </button>
            </div>
