...
const STATUS_TONE: Record<string, string> = {
  settled: 'text-[var(--brief-ink)]',
  confirmed: 'text-[var(--brief-ink)]',
  held: 'text-[var(--brief-ink)]',
  pending: 'text-[var(--brief-ink)]',
  created: 'text-[var(--ink-60)]',
  failed: 'text-[var(--brief-ink)]',
  refunded: 'text-[var(--brief-ink)]'
};
      ...
      <div>
        ...
        {ledger.status === 'ready' && (ledger.data ?? []).length === 0 && (
          <div className="border border-dashed border-[var(--brief-line)] rounded-2xl p-8 text-center">
            <p className="text-xs text-[var(--ink-60)]">No transactions yet.</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          {(ledger.data ?? []).map((tx) => (
            ...
            >
              ...
              <span className="text-xs font-extrabold text-[var(--brief-ink)] shrink-0">
