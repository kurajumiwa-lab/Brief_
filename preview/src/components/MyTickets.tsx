...
function TicketQr({ code, size = 176 }: { code: string; size?: number }) {
  ...
  if (!dataUrl) return <div className="w-44 h-44 bg-[#F0F2F5] border border-[#E5E8EC] rounded-xl" />;
...
export function MyTickets({ onSell }: { onSell?: () => void }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      <div>
        <h2 className="text-base font-extrabold text-[#0D1117]">My tickets</h2>
      ...
      {notice && (
        <div className="text-xs bg-[#F0F2F5] border border-[#E5E8EC] rounded-xl px-3 py-2.5 text-[#0D1117]">
          {notice}
        </div>
      )}

      {load.status === 'loading' && (
        <div className="text-xs text-[#0D1117]/70 py-8 text-center">Loading your tickets…</div>
      )}

      {load.status === 'error' && (
        <div className="border border-[#E5E8EC] rounded-xl p-4 text-center space-y-2">
          <p className="text-xs text-[#0D1117]/70">{load.error}</p>
          <button
            onClick={() => void fetchTickets()}
            className="text-xs font-bold text-[#0D1117] cursor-pointer underline"
          >
            Try again
          </button>
        </div>
      ...
      {load.status === 'ready' && load.data?.length === 0 && (
        <div className="border border-[#E5E8EC] rounded-xl p-6 text-center space-y-1">
          <p className="text-xs font-bold text-[#0D1117]">No tickets yet</p>
      ...
      {load.status === 'ready' && load.data?.map((t) => (
        <div key={t.id} className="border border-[#E5E8EC] rounded-2xl p-4 space-y-3 bg-[#FFFFFF]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-extrabold text-[#0D1117] truncate">{t.eventTitle ?? 'Event'}</p>
              <p className="text-[10px] text-[#0D1117]/70 mt-0.5">
                {t.status === 'void' ? 'Voided — this seat cannot be used' : `Code version ${t.codeVersion}`}
              </p>
            </div>
            {t.status === 'void' ? (
              <span className="shrink-0 text-[10px] font-bold px-2 py-1 rounded-full bg-[#EFF1F4] text-[#0D1117]/60">
                VOID
              </span>
            ) : t.activeListingId ? (
              <span className="shrink-0 text-[10px] font-bold px-2 py-1 rounded-full bg-[#EFF1F4] text-[#0D1117]/70">
                LISTED FOR RESALE
              </span>
            ) : null}
          </div>
          ...
          {t.status !== 'void' && (
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="min-w-0 space-y-1 text-center sm:text-left">
                <p className="font-mono text-sm font-bold text-[#0D1117] tracking-wide break-all">{t.scanCode}</p>
          ...
          {t.transfers.length > 0 && (
            <div className="border-t border-[#E5E8EC] pt-2 space-y-1">
              <p className="text-[10px] font-bold text-[#0D1117]/60 uppercase tracking-wide">This seat's history</p>
              {t.transfers.map((x, i) => (
                <p key={i} className="text-[10px] text-[#0D1117]/60">
          ...
          {t.status !== 'void' && !t.activeListingId && (
            <div className="flex flex-wrap gap-2 border-t border-[#E5E8EC] pt-3">
              {onSell && (
                <button
                  onClick={onSell}
              ...
              {giftFor === t.id ? (
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    ...
                    className="text-xs bg-[#FFFFFF] text-[#0D1117] rounded-xl px-3 py-2 border border-[#E5E8EC] focus:border-[#2563EB] focus:outline-none"
                  />
                  <button
                    disabled={busy || !handle.trim()}
                    onClick={() => void gift(t.id)}
                    className="text-xs font-bold px-3 py-2 rounded-xl border border-[#2563EB] text-[#0D1117] cursor-pointer disabled:opacity-40"
                  >
                    Gift
                  </button>
                  <button
                    onClick={() => { setGiftFor(null); setHandle(''); setGiftError(null); }}
                    className="text-xs text-[#0D1117]/60 cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <button
                  ...
                  className="text-xs font-bold px-3 py-2 rounded-xl border border-[#E5E8EC] text-[#0D1117] cursor-pointer"
                >
                  Gift to someone
                </button>
              )}
          ...
          {giftFor === t.id && giftError && (
            <p className="text-[11px] text-[#0D1117]">{giftError}</p>
          )}
        </div>
      ))}
    </div>
