...
const STATUS_STYLE: Record<string, string> = {
  draft: 'bg-[#222630] text-[#F7F7F8]/60',
      ...
      <div className="space-y-3">
        <div className="bg-[#12151A] border border-[#222630] rounded-2xl p-4 space-y-3">
          ...
          <input
            ...
            className="w-full bg-[#171A20] border border-[#222630] rounded-xl px-3 py-2 text-xs text-[#F7F7F8] outline-none"
          />
          <input
  ...
  return (
    <div className="space-y-4">
      ...
      {earnings && earnings.orderCount > 0 && (
        <div className="bg-[#12151A] border border-[#222630] rounded-2xl p-4 space-y-1">
          ...
          {!earnings.payoutAvailable && (
            <p className="text-[10px] text-[#F7F7F8]">{earnings.payoutReason}</p>
          )}
        </div>
      )}
      ...
      <div className="bg-[#12151A] border border-[#222630] rounded-2xl p-4 space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Price"
            inputMode="numeric"
            className="flex-1 bg-[#171A20] border border-[#222630] rounded-xl px-3 py-2 text-xs text-[#F7F7F8] outline-none"
          />
          <select
            value={listingDraft.type}
            onChange={(e) => onListingDraftChange({ type: e.target.value as Listing['type'] })}
            className="bg-[#171A20] border border-[#222630] rounded-xl px-3 py-2 text-xs text-[#F7F7F8] outline-none"
          >
      ...
      <div className="space-y-2">
        ...
        {listings.length === 0 ? (
          <p className="text-xs text-[#F7F7F8]/60">You have not listed anything yet.</p>
        ) : (
          listings.map((l) => (
            <div key={l.id} className="bg-[#12151A] border border-[#222630] rounded-2xl p-3 space-y-2">
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`shrink-0 text-[9px] px-2 py-0.5 rounded-full ${
                    STATUS_STYLE[l.status] ?? 'bg-[#222630] text-[#F7F7F8]/60'
                  }`}
                >
                  {l.status}
                </span>
              ...
              {(NEXT_ACTIONS[l.status] ?? []).length > 0 && (
                ...
                <button
                  ...
                  className="mr-1.5 cursor-pointer rounded-full border border-[#FF5A1F] px-3 py-1 text-[10px] font-extrabold text-[#FF5A1F] hover:bg-[#171A20]"
                >
                  Promote
                </button>
              )}
      ...
      <div className="space-y-2">
        ...
        {orders.length === 0 ? (
          <p className="text-xs text-[#F7F7F8]/60">No one has ordered from you yet.</p>
        ) : (
          orders.map((o) => (
            <OrderStatus
              key={o.id}
