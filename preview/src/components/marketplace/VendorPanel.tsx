...
const STATUS_STYLE: Record<string, string> = {
  draft: 'bg-[#E5E7EB] text-[#111111]/60',
  active: 'bg-[#FFFFFF] text-[#111111]',
  paused: 'bg-[#FFFFFF] text-[#111111]',
  sold_out: 'bg-[#FFFFFF] text-[#111111]',
  archived: 'bg-[#E5E7EB] text-[#111111]/40'
};
      ...
      <div className="space-y-3">
        <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-2xl p-4 space-y-3">
          ...
          <input
            ...
            className="w-full bg-[#FAFAFA] border border-[#E5E7EB] rounded-xl px-3 py-2 text-xs text-[#111111] outline-none"
          />
          <input
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-2xl p-4 space-y-1">
        <h4 className="text-sm font-extrabold text-[#111111]">{vendor.displayName}</h4>
        {vendor.description && <p className="text-xs text-[#111111]/60">{vendor.description}</p>}
        {vendor.verification.facts.map((f) => (
          <p key={f.kind} className="text-[10px] text-[#111111]/40">
            {f.label}
          </p>
        ))}
      </div>
      ...
      {earnings && earnings.orderCount > 0 && (
        <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-2xl p-4 space-y-1">
          ...
          {!earnings.payoutAvailable && (
            <p className="text-[10px] text-[#111111]">{earnings.payoutReason}</p>
          )}
        </div>
      )}
      ...
      <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-2xl p-4 space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Price"
            inputMode="numeric"
            className="flex-1 bg-[#FAFAFA] border border-[#E5E7EB] rounded-xl px-3 py-2 text-xs text-[#111111] outline-none"
          />
          <select
            value={listingDraft.type}
            onChange={(e) => onListingDraftChange({ type: e.target.value as Listing['type'] })}
            className="bg-[#FAFAFA] border border-[#E5E7EB] rounded-xl px-3 py-2 text-xs text-[#111111] outline-none"
          >
      ...
      <div className="space-y-2">
        ...
        {listings.length === 0 ? (
          <p className="text-xs text-[#111111]/60">You have not listed anything yet.</p>
        ) : (
          listings.map((l) => (
            <div key={l.id} className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-2xl p-3 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-extrabold text-[#111111]">{l.title}</p>
                  <p className="text-[10px] text-[#111111]/40">
                ...
                <span
                  className={`shrink-0 text-[9px] px-2 py-0.5 rounded-full ${
                    STATUS_STYLE[l.status] ?? 'bg-[#E5E7EB] text-[#111111]/60'
                  }`}
                >
                  {l.status}
                </span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(NEXT_ACTIONS[l.status] ?? []).map((a) => (
                    <button
                      ...
                      className="px-3 py-1 rounded-full bg-[#FFFFFF] text-[#111111] text-[10px] font-extrabold cursor-pointer disabled:opacity-50"
                    >
                      {a.label}
                    </button>
                  ))}
      ...
      <div className="space-y-2">
        ...
        {orders.length === 0 ? (
          <p className="text-xs text-[#111111]/60">No one has ordered from you yet.</p>
        ) : (
          orders.map((o) => (
            <OrderStatus
              key={o.id}
