...
async function main() {
  ...
  const mt = modalText();
  const chips = Array.from(document.querySelectorAll('.fixed.inset-0.z-50 span')).map(e=>e.textContent.trim());
  check('shows type label', chips.includes('Place'), `chips: ${chips.slice(0,4).join('|')}`);
  ...
  check('shows Hours fact', mt.includes('Hours') && mt.includes('06:00-18:30'));
  check('shows trust score', mt.includes('96%'));
  check('shows who provided it', mt.includes('City County Markets Board'));
  check('shows when it was checked', /checked \d{4}-\d{2}-\d{2}/.test(mt));
  check('does not overclaim accuracy', mt.includes('not a guarantee'));
