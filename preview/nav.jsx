...
async function main(){
  ...
  console.log('=== Three primary screens ===');
  const top=['Nearby','My Layer','Workflows'];
  ...
  const shelfIds = ['around', 'mshikano', 'events', 'create', 'share', 'groups'];
  check('main shelf has six labelled doors (incl. Mshikano)', shelfIds.every((id) => Boolean(document.querySelector(`[data-shelf-id="${id}"]`))));
  ...
  check('rail has Menu plus 3 screens plus Updates', !!rail && rail.querySelectorAll('button').length===5, rail?String(rail.querySelectorAll('button').length):'no rail');
  check('bar has Menu plus 3 screens', !!bar && bar.querySelectorAll('button').length===4);
  ...
  check('tapping bottom bar changes destination', /Your Layer|Things you.ve kept/i.test(body()));
  ...
  check('every rail item has an icon', rail.querySelectorAll('svg').length>=4);
