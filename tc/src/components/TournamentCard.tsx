...
export function TournamentCard({
  ...
  return (
    <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-3">
      <div className="flex items-baseline justify-between gap-3">
        <p className="text-xs font-extrabold text-[#251045]">{name}</p>
      ...
      {prizeDescription && (
        <p className="text-[10px] text-[#251045] mt-1">{prizeDescription}</p>
      )}

      {/* Organizer reward, itemised. Shown honestly, including zero. */}
      {rewardPoints > 0 ? (
        <div className="mt-2 pt-2 border-t border-[#D6CFE4]">
          <div className="flex items-baseline justify-between gap-3">
            <span className="text-[9px] text-[#251045]/40">Organizer earned</span>
            <span className="text-xs font-extrabold text-[#251045]">
              {rewardPoints.toLocaleString()}
            </span>
          </div>
          {rewardLines.map((l) => (
            <div key={l.label} className="flex items-baseline justify-between gap-3">
              <span className="text-[9px] text-[#251045]/60">{l.label}</span>
              <span className="text-[9px] text-[#251045]/60">+{l.points.toLocaleString()}</span>
            </div>
          ))}
        </div>
      ) : (
        <p className="text-[9px] text-[#251045]/60 mt-1">Organizer reward: none - {rewardReason}</p>
      )}

      {status !== 'completed' && (
        <p className="text-[9px] text-[#251045] mt-1">
      ...
      <button
        ...
        className="mt-2 w-full rounded-lg border border-[#D6CFE4] px-3 py-1.5 text-[10px] font-bold text-[#251045] transition-colors active:border-[#D6CFE4]"
      >
        {bracketOpen ? 'Hide bracket' : 'View bracket'}
      </button>
      {bracketOpen && (
        <div className="brief-rise-in mt-2 border-t border-[#D6CFE4] pt-2">
