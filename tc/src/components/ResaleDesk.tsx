...
export function ResaleDesk() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      <div>
        <h2 className="text-base font-extrabold text-[#111111]">Resale</h2>
      ...
      {notice && (
        <div className="text-xs bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl px-3 py-2.5 text-[#111111]">{notice}</div>
      )}
      {actionError && (
        <div className="text-xs border border-[#111111] rounded-xl px-3 py-2.5 text-[#111111]">{actionError}</div>
      )}

      {/* --- your seats ------------------------------------------------- */}
      <section className="space-y-3">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#111111]/50">Seats you can list</h3>
        {tickets.status === 'loading' && <p className="text-xs text-[#111111]/50">Loading your seats…</p>}
        {tickets.status === 'error' && (
          <div className="border border-[#E5E7EB] rounded-xl p-4 text-center space-y-2">
            <p className="text-xs text-[#111111]/70">{tickets.error}</p>
            <button onClick={() => void load()} className="text-xs font-bold text-[#111111] underline cursor-pointer">Try again</button>
          </div>
        )}
        {tickets.status === 'ready' && listable.length === 0 && (
          <div className="border border-[#E5E7EB] rounded-xl p-5 text-center">
            <p className="text-xs font-bold text-[#111111]">Nothing to list right now</p>
        ...
        {tickets.status === 'ready' && listable.map((t) => (
          <div key={t.id} className="border border-[#E5E7EB] rounded-2xl p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <p className="text-xs font-extrabold text-[#111111] truncate">{t.eventTitle ?? 'Event'}</p>
              <span className="text-[10px] text-[#111111]/50 shrink-0">code v{t.codeVersion}</span>
            </div>
            {listFor === t.id ? (
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    ...
                    className="text-xs bg-[#FFFFFF] text-[#111111] rounded-xl px-3 py-2 border border-[#E5E7EB] focus:border-[#111111] focus:outline-none w-40"
                  />
                  <input
                    value={note}
                    onChange={(e) => setNote(e.target.value)}
                    placeholder="why you're selling (optional)"
                    className="flex-1 min-w-40 text-xs bg-[#FFFFFF] text-[#111111] rounded-xl px-3 py-2 border border-[#E5E7EB] focus:border-[#111111] focus:outline-none"
                  />
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="text-xs font-bold px-3 py-2 rounded-xl bg-[#111111] text-[#FFFFFF] cursor-pointer disabled:opacity-40"
                  >
                    Put it up for sale
                  </button>
                  <button onClick={() => { setListFor(null); setPrice(''); setNote(''); }} className="text-xs text-[#111111]/60 cursor-pointer">
                    Not now
                  </button>
                </div>
              ...
              <button
                ...
                className="text-xs font-bold px-3 py-2 rounded-xl bg-[#111111] text-[#FFFFFF] cursor-pointer"
              >
                List this seat
              </button>
            )}
      ...
      {/* --- active listings --------------------------------------------- */}
      <section className="space-y-3">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#111111]/50">Your listings</h3>
        {desk.status === 'loading' && <p className="text-xs text-[#111111]/50">Loading listings…</p>}
        {desk.status === 'error' && <p className="text-xs text-[#111111]/70">{desk.error}</p>}
        {desk.status === 'ready' && activeListings.length === 0 && (
          <p className="text-[11px] text-[#111111]/60 border border-[#E5E7EB] rounded-xl p-4">
        ...
        {desk.status === 'ready' && activeListings.map((l) => {
          ...
          return (
            <div key={l.id} className="border border-[#E5E7EB] rounded-2xl p-4 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-extrabold text-[#111111]">{money(l.price, l.currency)}</p>
                  <p className="text-[10px] text-[#111111]/50 mt-0.5">{LISTING_LABEL[l.status] ?? l.status}</p>
                </div>
                {l.status === 'active' && (
                  <button
                    disabled={busy}
                    onClick={() => void act(() => briefApi.cancelResaleListing(l.id), 'Listing pulled. The seat is yours to list again.')}
                    className="text-xs px-3 py-2 rounded-xl border border-[#E5E7EB] text-[#111111] cursor-pointer disabled:opacity-40"
                  >
                    Pull listing
                  </button>
                )}
        ...
        {desk.status === 'ready' && finishedListings.length > 0 && (
          <div className="space-y-1">
            {finishedListings.map((l) => (
              <p key={l.id} className="text-[10px] text-[#111111]/50 flex items-center justify-between border-b border-[#F3F4F6] pb-1">
      ...
      <section className="space-y-3">
        <h3 className="text-[10px] font-bold uppercase tracking-wide text-[#111111]/50">Seats you are buying</h3>
        {desk.status !== 'ready' && <p className="text-xs text-[#111111]/50">Loading orders…</p>}
        {(() => {
          ...
          return asBuyer.map((o) => (
            <div key={o.id} className="border border-[#E5E7EB] rounded-2xl p-4 space-y-2">
              ...
              {o.status === 'pending' ? (
                ...
                <p className="text-[11px] text-[#111111]/70">This order is closed.</p>
              )}
            </div>
          ));
        })()}
