...
function ChannelStatus({ glyph, name, detail, configured, online, hue }: {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-[#1E1E22] bg-[#121214] px-4 py-3">
      ...
      <span
        className={`flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-wide ${
          live ? 'bg-[#00E676]/15 text-[#00E676]' : configured ? 'bg-[#1C1C1F] text-[#8E8E93]' : 'bg-[#1C1C1F] text-[#48484A]'
        }`}
      >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <ChannelStatus glyph="📡" name="Telegram" detail="Groups and channels, in real time" configured={tg} online={online} hue="#00E676" />
        <ChannelStatus glyph="💬" name="WhatsApp" detail="Inbound messages, signature-verified" configured={wa} online={online} hue="#00E676" />
        <ChannelStatus glyph="🌐" name="Web & RSS" detail="Public pages and feeds — always on" configured online={online} hue="#00E676" />
        <ChannelStatus glyph="✍️" name="Manual capture" detail="Paste anything, anywhere — always on" configured online hue="#00E676" />
      </div>

      {/* 3. Reachability — the only "live" system signal, stated plainly */}
      <div className="flex items-center justify-between rounded-2xl border border-[#1E1E22] bg-[#121214] px-4 py-3">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${online ? 'bg-[#00E676]' : 'bg-[#48484A]'}`} />
          <p className="text-[12px] font-bold text-[#FFFFFF]">Ingestion engine</p>
        </div>
