...
export default function ServiceFees() {
  ...
  return (
    <div className="space-y-4">
      <section aria-label="Pay for Brief services" className="rounded-2xl border border-[#E5E8EC] bg-[#FFFFFF] p-4 space-y-3">
        ...
        <div className="space-y-2">
          {(data?.services ?? []).map((svcItem) => (
            <label key={svcItem.key} className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-2 cursor-pointer ${service === svcItem.key ? 'border-[#FF5A1F] bg-[#F0F2F5]' : 'border-[#E5E8EC]'}`}>
          ...
          {data && data.services.length === 0 && (
            <p className="text-[11px] text-[#0D1117]/60">No services are priced yet.</p>
          )}
        </div>
      ...
      <section aria-label="Your payments" className="rounded-2xl border border-[#E5E8EC] bg-[#FFFFFF] p-4 space-y-2">
        <h2 className="text-[13px] font-extrabold text-[#0D1117]">Your payments</h2>
        {(data?.fees ?? []).length === 0 && (
          <p className="text-[11px] text-[#0D1117]/60">Nothing yet. Your first payment will show here, with its state.</p>
        )}
        {(data?.fees ?? []).slice(0, 10).map((f) => (
          <div key={f.id} className="rounded-xl border border-[#E5E8EC] px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[11px] font-bold text-[#0D1117]">{f.label}</p>
