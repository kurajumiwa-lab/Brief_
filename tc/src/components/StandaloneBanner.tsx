...
export function StandaloneBanner({ banner }: StandaloneBannerProps) {
  ...
  return (
    <article className="group relative min-h-[190px] overflow-hidden rounded-2xl border border-[#222630] bg-[#FF5A1F]">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#08090B]/95 via-[#08090B]/55 to-[#08090B]/10" />
      <div className="relative flex min-h-[190px] flex-col justify-between p-4 text-[#F7F7F8]">
        <div className="flex items-center justify-between gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#12151A] px-2.5 py-1 text-[8px] font-extrabold uppercase tracking-[0.16em] text-[#F7F7F8]">
            <Sparkles className="h-3 w-3" /> Standalone banner
          </span>
          {banner.location && <span className="max-w-[45%] truncate text-[9px] font-bold text-[#F7F7F8]/70">{banner.location}</span>}
        </div>
        <div>
          <h3 className="max-w-xl text-[18px] font-extrabold leading-tight">{banner.title}</h3>
          {banner.body && <p className="mt-1 line-clamp-2 max-w-xl text-[11px] leading-snug text-[#F7F7F8]/75">{banner.body}</p>}
            ...
            {banner.share.available ? (
              <>
                <a
                  ...
                  className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-[#12151A] px-3 py-2 text-[10px] font-extrabold text-[#F7F7F8]"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer noopener"
                  className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-[#222630]/30 px-3 py-2 text-[10px] font-extrabold text-[#F7F7F8]"
                >
                  <ExternalLink className="h-3.5 w-3.5" /> View campaign
                </a>
              </>
            ) : (
              <span className="rounded-lg border border-[#222630]/25 px-3 py-2 text-[10px] font-bold text-[#F7F7F8]/70">
