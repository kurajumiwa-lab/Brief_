...
function Row({ children }: { children: React.ReactNode }) {
  return <div className="flex items-center justify-between gap-2 text-[11px] text-[#F7F7F8]/80">{children}</div>;
}
...
export function AdminDesk({ open, onClose, me }: { open: boolean; onClose: () => void; me: AuthedUser | null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#FF5A1F]/30 backdrop-blur-sm overflow-y-auto">
      <div className="max-w-3xl mx-auto min-h-full bg-[#171A20]">
        <div className="sticky top-0 z-10 bg-[#171A20]/95 border-b border-[#222630] px-4 pt-5 pb-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-extrabold text-[#F7F7F8]">Operate</h2>
            ...
            <button onClick={onClose} aria-label="Close the operator desk"
              className="shrink-0 rounded-xl border border-[#222630] bg-[#12151A] p-2 cursor-pointer">
              <X className="w-4 h-4 text-[#F7F7F8]" />
            </button>
          </div>
        ...
        <>
                ...
                {Object.keys(funnel.funnel).length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {Object.entries(funnel.funnel).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, n]) => (
                      <span key={name} className="rounded-full bg-[#171A20] px-2 py-0.5 text-[9px] font-bold text-[#F7F7F8]/70">{name} · {n}</span>
                    ))}
                  </div>
                )}
              </>
          ...
          <Card title="Directory" note="Search by handle or name. Suspended members are locked out on their next request, not their next login.">
            <input
              ...
              className="w-full rounded-lg border border-[#222630] bg-[#12151A] px-2.5 py-1.5 text-[11px] outline-none focus:border-[#FF5A1F]"
            />
              ...
              {selected.status === 'active' ? (
                <div className="space-y-1.5 pt-2">
                  <input value={reason} onChange={(e) => setReason(e.target.value)}
                    ...
                    className="w-full rounded-lg border border-[#222630] bg-[#12151A] px-2.5 py-1.5 text-[10px] outline-none focus:border-[#FF5A1F]" />
                  <button type="button" onClick={() => suspend(selected)} disabled={busy || reason.trim().length < 4}
                    className="rounded-lg border border-[#FF5D6C]/30 bg-[#FF5D6C]/5 px-3 py-1.5 text-[10px] font-extrabold text-[#FF5D6C] cursor-pointer disabled:opacity-40">
                    Suspend — locks them out now
                  </button>
                </div>
              ) : (
                <button type="button" onClick={() => reinstate(selected)} disabled={busy}
                  className="mt-2 rounded-lg border border-[#38E879]/30 bg-[#38E879]/5 px-3 py-1.5 text-[10px] font-extrabold text-[#38E879] cursor-pointer disabled:opacity-40">
                  Reinstate
                </button>
              )}
            </Card>
    ...
    <>
      ...
      {can('ops.run') ? (
        <button onClick={() => void backup()}
          className="w-full rounded-xl bg-[#FF5A1F] px-3 py-2.5 text-[11px] font-extrabold text-[#0D0F12] cursor-pointer">
    ...
    <>
              ...
              {can('moderate') ? (
                <div className="flex gap-1.5 pt-1">
                  <button onClick={() => void act(() => briefApi.resolveOpsReport(r.id, 'dismiss', 'reviewed at the desk'), 'dismissed')}
                    className="rounded-lg border border-[#222630] px-2 py-1 text-[9px] font-extrabold cursor-pointer">Dismiss</button>
                  <button onClick={() => void act(() => briefApi.resolveOpsReport(r.id, 'remove', 'removed from discovery at the desk'), 'removed from discovery')}
                    className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer">Remove from discovery</button>
                </div>
              ...
              {c.status === 'applied' && can('moderate') && (
                <div className="flex gap-1.5 pt-1">
                  <button onClick={() => void act(() => briefApi.opsRejectCorrection(c.id, 'rejected at the desk'), 'correction rejected')}
                    className="rounded-lg border border-[#222630] px-2 py-1 text-[9px] font-extrabold cursor-pointer">Reject</button>
                </div>
              )}
            </div>
          ))}
        {can('moderate') && (
          <div className="rounded-xl border border-[#222630] p-2.5 space-y-1.5">
            <p className="text-[10px] font-extrabold text-[#F7F7F8]/60">Correct a field</p>
            <div className="flex flex-wrap gap-1.5">
              <input
                value={corrObjectId}
                onChange={(e) => setCorrObjectId(e.target.value)}
                placeholder="object id"
                className="min-w-0 flex-1 rounded-lg border border-[#222630] px-2 py-1 text-[10px]"
              />
              <select value={corrField} onChange={(e) => setCorrField(e.target.value)} className="rounded-lg border border-[#222630] px-2 py-1 text-[10px]">
              ...
              <button
                ...
                className="rounded-lg bg-[#FF5A1F] px-2.5 py-1 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer"
              >
                Apply
              </button>
            </div>
      ...
      <Card title="Source trust" note="An operator decision about a source's standing — never a public rating, never 'this source is always true'. Degraded ranks lower; disabled stops contributing to the default feed (content stays reachable directly).">
        {trustSources === null ? <Empty>loading…</Empty>
          ...
          : trustSources.map((s) => (
            <div key={s.id} className="rounded-xl border border-[#222630] p-2.5 space-y-1">
              <Row>
  ...
  return (
    <div className="space-y-1.5 pt-1">
      <input value={reason} onChange={(e) => setReason(e.target.value)}
        ...
        className="w-full rounded-lg border border-[#222630] px-2 py-1.5 text-[10px]" />
      <div className="flex gap-1.5">
        <button onClick={() => onDecide('approved', reason || 'approved at the desk')}
          className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer">Approve</button>
        <button onClick={() => onDecide('rejected', reason)}
          disabled={!reason.trim()}
          className="rounded-lg border border-[#222630] px-2 py-1 text-[9px] font-extrabold cursor-pointer disabled:opacity-40">Reject</button>
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <Card title="Ingest status" note="What the sources layer reports about itself right now.">
        {ingest === null ? <Empty>loading…</Empty> : (
          <pre className="overflow-x-auto rounded-xl bg-[#FF5A1F] px-3 py-2 text-[9px] leading-relaxed text-[#0D0F12]/90">
                ...
                <div className="flex gap-1.5 pt-1">
                  {['publish', 'archive'].map((a) => (
                    <button key={a} onClick={() => void transition(String(c.key), a)}
                      className="rounded-lg border border-[#222630] px-2 py-1 text-[9px] font-extrabold cursor-pointer">{a}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        {!can('admin') ? <Empty>Seeding needs the admin capability.</Empty> : (
          <div className="flex gap-1.5">
            <button onClick={async () => { const r = await briefApi.seedDemo(); setNote(r.ok ? 'demo content seeded' : r.error); refresh(); }}
              className="rounded-lg border border-[#222630] px-2 py-1 text-[9px] font-extrabold cursor-pointer">Seed demo</button>
            <button onClick={async () => { const r = await briefApi.clearDemo(); setNote(r.ok ? 'demo content cleared' : r.error); refresh(); }}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer">Clear demo</button>
          </div>
        )}
      </Card>
    </>
          ...
          <div className="space-y-1.5">
            {(['kind', 'key', 'url', 'alt'] as const).map((f) => (
              <input key={f} value={(form as any)[f]} onChange={(e) => setForm({ ...form, [f]: e.target.value })}
                placeholder={f} className="w-full rounded-lg border border-[#222630] px-2 py-1.5 text-[10px]" />
            ))}
            <button onClick={() => void record()}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1.5 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer">Record</button>
          </div>
        </Card>
...
function CommerceTab({ tick, can, refresh }: { tick: number; can: (c: string) => boolean; refresh: () => void }) {
  ...
  return (
    <>
      ...
      <button onClick={refresh} className="w-full rounded-xl border border-[#222630] bg-[#12151A] px-3 py-2 text-[11px] font-extrabold cursor-pointer">
        ...
        {!can('admin') ? <Empty>Assigning roles needs the admin capability.</Empty> : (
          <div className="space-y-1.5">
            <input value={roles.userId} onChange={(e) => setRoles({ ...roles, userId: e.target.value })}
              placeholder="user id or handle" className="w-full rounded-lg border border-[#222630] px-2 py-1.5 text-[10px]" />
            <input value={roles.roles} onChange={(e) => setRoles({ ...roles, roles: e.target.value })}
              placeholder="roles (viewer, operator, reviewer, finance, admin)" className="w-full rounded-lg border border-[#222630] px-2 py-1.5 text-[10px]" />
            <input value={roles.reason} onChange={(e) => setRoles({ ...roles, reason: e.target.value })}
              placeholder="reason (written to the audit log)" className="w-full rounded-lg border border-[#222630] px-2 py-1.5 text-[10px]" />
            <button onClick={() => void submit()} disabled={!roles.userId.trim() || !roles.reason.trim()}
              className="rounded-lg bg-[#FF5A1F] px-2 py-1.5 text-[9px] font-extrabold text-[#0D0F12] cursor-pointer disabled:opacity-40">
              Write roles
            </button>
          </div>
        )}
