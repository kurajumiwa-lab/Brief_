...
const STATUS_STYLE: Record<string, string> = {
  ordered: 'bg-[#FBFAFD] text-[#251045]',
  fulfilled: 'bg-[#FBFAFD] text-[#251045]',
  settled: 'bg-[#FBFAFD] text-[#251045]',
  disputed: 'bg-[#FBFAFD] text-[#251045]',
  cancelled: 'bg-[#D6CFE4] text-[#251045]/60',
  offered: 'bg-[#D6CFE4] text-[#251045]/60'
};
...
export function OrderStatus({
  ...
  return (
    <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-3 space-y-2">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-extrabold text-[#251045]">{order.listingTitle}</p>
        ...
        <span
          className={`shrink-0 text-[9px] px-2 py-0.5 rounded-full ${
            STATUS_STYLE[order.status] ?? 'bg-[#D6CFE4] text-[#251045]/60'
          }`}
        >
      ...
      {order.dispute && (
        <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-xl p-2">
          <p className="text-[10px] font-extrabold text-[#251045]">Disputed</p>
          <p className="text-[10px] text-[#251045]/60">{order.dispute.reason}</p>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-1.5 pt-0.5">
          {canSettle && onSettle && (
            <button
              ...
              disabled={busy}
              className="px-3 py-1 rounded-full bg-[#5B2EA6] text-[#FFFFFF] text-[10px] font-extrabold cursor-pointer disabled:opacity-50"
            >
              Settle
            </button>
          )}
          ...
          {canDispute && onDispute && (
            <button
              ...
              disabled={busy}
              className="px-3 py-1 rounded-full bg-[#FBFAFD] text-[#251045] border border-[#D6CFE4] text-[10px] font-extrabold cursor-pointer disabled:opacity-50"
            >
              Report a problem
            </button>
          )}
