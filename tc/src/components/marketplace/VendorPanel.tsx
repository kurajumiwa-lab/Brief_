...
const STATUS_STYLE: Record<string, string> = {
  draft: 'bg-[#D6CFE4] text-[#251045]/60',
  active: 'bg-[#FBFAFD] text-[#251045]',
  paused: 'bg-[#FBFAFD] text-[#251045]',
  sold_out: 'bg-[#FBFAFD] text-[#251045]',
  archived: 'bg-[#D6CFE4] text-[#251045]/40'
};
      ...
      <div className="space-y-3">
        <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-4 space-y-3">
          ...
          <input
            ...
            className="w-full bg-[#F1EDF7] border border-[#D6CFE4] rounded-xl px-3 py-2 text-xs text-[#251045] outline-none"
          />
          <input
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-4 space-y-1">
        <h4 className="text-sm font-extrabold text-[#251045]">{vendor.displayName}</h4>
        {vendor.description && <p className="text-xs text-[#251045]/60">{vendor.description}</p>}
        {vendor.verification.facts.map((f) => (
          <p key={f.kind} className="text-[10px] text-[#251045]/40">
            {f.label}
          </p>
        ))}
      </div>
      ...
      {earnings && earnings.orderCount > 0 && (
        <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-4 space-y-1">
          ...
          {!earnings.payoutAvailable && (
            <p className="text-[10px] text-[#251045]">{earnings.payoutReason}</p>
          )}
        </div>
      )}
      ...
      <div className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-4 space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Price"
            inputMode="numeric"
            className="flex-1 bg-[#F1EDF7] border border-[#D6CFE4] rounded-xl px-3 py-2 text-xs text-[#251045] outline-none"
          />
          <select
            value={listingDraft.type}
            onChange={(e) => onListingDraftChange({ type: e.target.value as Listing['type'] })}
            className="bg-[#F1EDF7] border border-[#D6CFE4] rounded-xl px-3 py-2 text-xs text-[#251045] outline-none"
          >
      ...
      <div className="space-y-2">
        ...
        {listings.length === 0 ? (
          <p className="text-xs text-[#251045]/60">You have not listed anything yet.</p>
        ) : (
          listings.map((l) => (
            <div key={l.id} className="bg-[#FBFAFD] border border-[#D6CFE4] rounded-2xl p-3 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-extrabold text-[#251045]">{l.title}</p>
                  <p className="text-[10px] text-[#251045]/40">
                ...
                <span
                  className={`shrink-0 text-[9px] px-2 py-0.5 rounded-full ${
                    STATUS_STYLE[l.status] ?? 'bg-[#D6CFE4] text-[#251045]/60'
                  }`}
                >
                  {l.status}
                </span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(NEXT_ACTIONS[l.status] ?? []).map((a) => (
                    <button
                      ...
                      className="px-3 py-1 rounded-full bg-[#FBFAFD] text-[#251045] text-[10px] font-extrabold cursor-pointer disabled:opacity-50"
                    >
                      {a.label}
                    </button>
                  ))}
      ...
      <div className="space-y-2">
        ...
        {orders.length === 0 ? (
          <p className="text-xs text-[#251045]/60">No one has ordered from you yet.</p>
        ) : (
          orders.map((o) => (
            <OrderStatus
              key={o.id}
