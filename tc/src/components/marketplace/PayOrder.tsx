...
const STATUS_TONE: Record<string, string> = {
  intent: 'text-[#E8D48B]',
  authorized: 'text-[#E8D48B]',
  confirmed: 'text-[#00FF42]',
  failed: 'text-[#C4553E]',
  cancelled: 'text-[#C4553E]',
  reversed: 'text-[#C4553E]'
};
...
export function PayOrder({ order, onPaid }: PayOrderProps) {
  ...
  return (
    <div className="bg-[#0B1A12] border border-[#1E3A2A] rounded-xl p-3 space-y-2">
      ...
      {error && <p className="text-[10px] text-[#C4553E]">{error}</p>}

      {status && (
        <p className={`text-[11px] font-bold ${STATUS_TONE[status] ?? 'text-[#86935C]'}`}>
      ...
      {!inFlight && (
        <div className="space-y-1.5">
          ...
          <button
            ...
            className="w-full px-3 py-2 rounded-lg bg-[#00FF42] text-[#09150E] font-extrabold text-[11px] cursor-pointer disabled:opacity-40"
          >
