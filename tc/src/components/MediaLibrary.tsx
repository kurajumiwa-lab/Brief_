  ...
  return (
    <div className="rounded-xl border border-[#E5E7EB] bg-[#FAFAFA] p-3 space-y-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[9px] font-extrabold uppercase tracking-[0.16em] text-[#111111]/60">
          Your media library
        </p>
        <button
          onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <>
          {error && (
            <p className="text-[10px] text-[#111111]">{error}</p>
          )}
          {rows === null && (
            <p className="text-[10px] text-[#111111]/60">Loading…</p>
          )}
          ...
          {rows !== null && rows.length > 0 && (
            <ul className="space-y-1.5 max-h-56 overflow-y-auto">
              {rows.map((m) => (
                <li
                  key={m.id}
                  className="flex items-center gap-2 rounded-lg bg-[#FFFFFF] border border-[#E5E7EB] px-2 py-1.5"
                >
                  <img
                    src={m.url}
                    alt=""
                    className="h-8 w-8 shrink-0 rounded-md object-cover bg-[#FAFAFA]"
                  />
                  ...
                  <button
                    onClick={() => onUse(m.url)}
                    className="shrink-0 px-2 py-1 rounded-lg bg-[#111111] text-[#FFFFFF] font-extrabold text-[9px] cursor-pointer"
                  >
                    Use
                  </button>
                  <button
                    ...
                    className="shrink-0 px-2 py-1 rounded-lg border border-[#E5E7EB] font-bold text-[9px] text-[#111111]/60 cursor-pointer disabled:opacity-50"
                  >
                    {busyId === m.id ? '…' : 'Delete'}
                  </button>
                </li>
          ...
          {note && <p className="text-[10px] text-[#111111]/70">{note}</p>}
        </>
      )}
    </div>
  );
