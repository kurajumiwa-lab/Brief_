...
function Row({ item }: { item: WireItem }) {
  return (
    <a
      ...
      className="group flex min-h-16 items-center gap-3 rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-2 transition-colors hover:border-[#6C3EC9]"
    >
      {item.image && (
        <img src={item.image} alt="" aria-hidden="true" loading="lazy" className="h-12 w-12 shrink-0 rounded-xl object-cover" />
      )}
      <h3 className="min-w-0 truncate px-1 text-[14px] font-semibold leading-snug text-[#251045]">
        {item.title}
      </h3>
    </a>
  );
...
export function WireSection() {
  ...
  if (state.status === 'loading') {
    return (
      <div className="grid grid-cols-2 gap-3 animate-pulse" aria-label="Loading">
        <div className="h-16 rounded-2xl bg-[#FBFAFD]" />
        <div className="h-16 rounded-2xl bg-[#FBFAFD]" />
      </div>
    );
  }
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-[#D6CFE4] bg-[#F1EDF7] p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="px-1 text-[11px] font-bold uppercase tracking-[0.22em] text-[#251045]/60">News</h2>
          <p className="mt-1 px-1 text-[10px] leading-snug text-[#251045]/45">
        ...
        <button
          ...
          disabled={refreshing}
          className="min-h-9 rounded-lg border border-[#D6CFE4] bg-[#FBFAFD] px-3 py-1.5 text-[10px] font-extrabold text-[#251045] disabled:opacity-50"
        >
      ...
      {wire?.error && (
        <p className="rounded-xl border border-dashed border-[#D6CFE4] bg-[#FBFAFD] px-3 py-2 text-[10px] leading-snug text-[#251045]/60">
          The live news request returned an error: {wire.error}
        </p>
      )}

      {headlines.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[#D6CFE4] bg-[#FBFAFD] px-3 py-3">
          <p className="text-[11px] font-extrabold text-[#251045]">No current news returned.</p>
        ...
        <>
          {wire?.kenya?.length ? (
            <div className="space-y-2">
              <p className="px-1 text-[9px] font-extrabold uppercase tracking-[0.16em] text-[#251045]/45">Kenya</p>
          ...
          {wire?.world?.length ? (
            <div className="space-y-2">
              <p className="px-1 text-[9px] font-extrabold uppercase tracking-[0.16em] text-[#251045]/45">World</p>
              <div className="grid gap-2 sm:grid-cols-2">
