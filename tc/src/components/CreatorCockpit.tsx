  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-extrabold text-[#F7F7F8]">Creator Cockpit</h2>
        ...
        <button onClick={() => void sweep()} className="flex items-center gap-1 rounded-full border border-[#222630] px-3 py-1.5 text-[11px] font-bold text-[#F7F7F8] cursor-pointer">
      ...
      <div className="rounded-2xl border border-[#222630] bg-[#12151A] p-4 space-y-3">
        <p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-[#F7F7F8]">
        ...
        <input
          ...
          className="w-full rounded-lg border border-[#222630] bg-[#171A20] px-3 py-2 text-[13px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]"
        />
        <div className="flex flex-wrap gap-2 text-[11px]">
          <div className="flex items-center gap-1.5">
            <span className="text-[#F7F7F8]/60">When</span>
            <select value={trigger} onChange={(e) => setTrigger(e.target.value)} className="rounded-lg border border-[#222630] bg-[#171A20] px-2 py-1.5 text-[#F7F7F8]">
              {TRIGGERS.map((t) => <option key={t} value={t}>{t === '*' ? 'Anything happens' : t}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-[#F7F7F8]/60">do</span>
            <select value={actionType} onChange={(e) => setActionType(e.target.value)} className="rounded-lg border border-[#222630] bg-[#171A20] px-2 py-1.5 text-[#F7F7F8]">
              {ACTION_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
        </div>
        {actionType === 'tag' ? (
          <input value={actionTitle} onChange={(e) => setActionTitle(e.target.value)} placeholder="Tag (e.g. Outdoor enthusiast)" className="w-full rounded-lg border border-[#222630] bg-[#171A20] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]" />
        ) : actionType === 'blast' ? (
          <input value={actionBody} onChange={(e) => setActionBody(e.target.value)} placeholder="Message text" className="w-full rounded-lg border border-[#222630] bg-[#171A20] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]" />
        ) : (
          <>
            <input value={actionTitle} onChange={(e) => setActionTitle(e.target.value)} placeholder="Notification title" className="w-full rounded-lg border border-[#222630] bg-[#171A20] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]" />
            <input value={actionBody} onChange={(e) => setActionBody(e.target.value)} placeholder="Notification body (optional)" className="w-full rounded-lg border border-[#222630] bg-[#171A20] px-3 py-2 text-[12px] text-[#F7F7F8] outline-none focus:border-[#22E6E0]" />
          </>
        )}
        <button onClick={() => void create()} disabled={busy || !name.trim()} className="flex items-center gap-1.5 rounded-full bg-[#FF5A1F] px-4 py-2 text-[12px] font-bold text-[#0D0F12] disabled:opacity-40 cursor-pointer">
      ...
      <div className="space-y-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#F7F7F8]">Your automations ({workflows.length})</p>
        {state === 'loading' && <p className="text-xs text-[#F7F7F8]/60">Loading…</p>}
        {state === 'ready' && workflows.length === 0 && (
          <p className="text-xs text-[#F7F7F8]/60">No automations yet. Create one above.</p>
        )}
        {workflows.map((w) => (
          <div key={w.id} className="flex items-center gap-3 rounded-xl border border-[#222630] bg-[#12151A] p-3">
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-semibold text-[#F7F7F8] truncate">{w.name}</p>
              <p className="text-[10px] text-[#F7F7F8]/60">
                when <span className="text-[#F7F7F8]">{w.trigger === '*' ? 'anything' : w.trigger}</span> → {w.actions.map((a) => a.type).join(', ')}
              </p>
            </div>
            <button
