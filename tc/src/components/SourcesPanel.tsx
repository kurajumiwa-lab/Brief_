...
export function SourcesPanel({
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-extrabold text-[#0D1117]">Sources</h2>
      ...
      <div className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4">
        <p className="text-[10px] text-[#0D1117]">Brief it</p>
        <p className="text-[11px] text-[#0D1117]/60 mt-1 leading-snug">
        ...
        <textarea
          ...
          className="w-full mt-2 bg-[#F0F2F5] border border-[#E5E8EC] rounded-xl p-3 text-xs text-[#0D1117] placeholder:text-[#0D1117]/60"
        />
        <div className="flex items-center gap-2 mt-2">
          <button
            onClick={runBriefItPreview}
            disabled={briefItBusy || !briefItText.trim()}
            className="px-3 py-1.5 rounded-full text-[11px] font-extrabold bg-[#FF5A1F] text-[#0D1117] cursor-pointer disabled:opacity-40"
          >
          ...
          {briefItPreview && !briefItPreview.error && briefItPreview.worthy && (
            <button
              ...
              className="px-3 py-1.5 rounded-full text-[11px] font-extrabold bg-[#FFFFFF] text-[#0D1117] border border-[#E5E8EC] cursor-pointer"
            >
              Save to Brief
            </button>
          )}
          {briefItPreview && (
            <button
              ...
              className="text-[11px] font-extrabold text-[#0D1117]/60 cursor-pointer"
            >
              Discard
            </button>
          )}
        ...
        {briefItSaved && (
          <p className="text-[11px] text-[#0D1117] mt-2">{briefItSaved}</p>
        )}

        {briefItPreview?.error && (
          <p className="text-[11px] text-[#0D1117] mt-2">{briefItPreview.error}</p>
        )}

        {briefItPreview && !briefItPreview.error && (
          <div className="mt-3 bg-[#F0F2F5] border border-[#E5E8EC] rounded-xl p-3">
            {!briefItPreview.worthy ? (
              ...
              <>
                <p className="text-[10px] font-extrabold text-[#0D1117]">
                  Found
                </p>
                <div className="mt-1.5 space-y-1">
                  {Object.entries(briefItPreview.fields ?? {}).map(([k, v]) => (
                    <div key={k} className="flex items-baseline justify-between gap-3">
                      <span className="text-[10px] text-[#0D1117]/60">{k}</span>
                      <span className="text-[11px] text-[#0D1117] text-right truncate">
                ...
                {(briefItPreview.vendors?.length > 0 || briefItPreview.products?.length > 0) && (
                  <div className="mt-2 pt-2 border-t border-[#E5E8EC] space-y-0.5">
                    {briefItPreview.vendors?.map((v: string) => (
                      <p key={v} className="text-[10px] text-[#0D1117]">Vendor: {v}</p>
                    ))}
                    {briefItPreview.products?.map((pr: any) => (
                      <p key={pr.name} className="text-[10px] text-[#0D1117]">
                        Product: {pr.name} - {pr.currency} {pr.price.toLocaleString()}
                      </p>
                    ))}
                  </div>
                )}
                <p className="text-[9px] text-[#0D1117]/60 mt-2">
      ...
      <div className="bg-[#FFFFFF] border border-[#E5E8EC] rounded-2xl p-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[10px] text-[#0D1117]">Connectors</p>
          <button
        ...
        {!connectorStatus.checked && (
          <p className="text-[11px] text-[#0D1117]/60 mt-2">Checking...</p>
        )}

        {connectorStatus.checked && !connectorStatus.online && (
          <p className="text-[11px] text-[#0D1117] mt-2 leading-snug">
            Ingestion server not reachable. Brief still works -- only live
            connectors are unavailable. Start it with{' '}
            <span className="text-[#0D1117]">npm start</span> in
          ...
          <>
            <div className="mt-2 space-y-2">
              {Object.entries(connectorStatus.capabilities).map(([name, cap]: [string, any]) => {
                ...
                return (
                  <div key={name} className="bg-[#F0F2F5] border border-[#E5E8EC] rounded-xl p-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-extrabold text-[#0D1117] capitalize">{name}</span>
                      <span
                        className={`text-[9px] font-extrabold ${
                          configured === false ? 'text-[#0D1117]' : 'text-[#0D1117]'
                        }`}
                      >
                    ...
                    {cap.receive && (
                      <p className="text-[10px] text-[#0D1117]/60 mt-1">Receive: {cap.receive}</p>
                    )}
                    {/* Failed capabilities are shown, not hidden (spec 27). */}
                    {unsupported.map(([k, v]) => (
                      <p key={k} className="text-[10px] text-[#0D1117] mt-0.5">
                        {k}: {String(v)}
                      </p>
                    ))}
                  </div>
            ...
            {connectorStatus.liveSources.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[#E5E8EC]">
                <p className="text-[9px] font-extrabold text-[#0D1117]">
                  Connected sources
                </p>
                <div className="mt-1.5 space-y-1">
                  {connectorStatus.liveSources.map((src: any) => (
                    <div key={src.id} className="flex items-baseline justify-between gap-3">
                      <span className="text-[11px] text-[#0D1117] truncate">{src.name}</span>
