...
function CapRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-2 py-1">
      ...
      <span className="text-[10px] font-mono font-bold text-[#251045]">{value}</span>
    </div>
  );
}
...
export function TierGuardrail({ guardrail }: TierGuardrailProps) {
  ...
  if (!guardrail) {
    return (
      <div className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4">
        <p className="text-[10px] text-[#251045]/40">Tier guardrail unavailable.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-[#D6CFE4] bg-[#FBFAFD] p-4 space-y-3">
      ...
      <div className="divide-y divide-[#D6CFE4]">
      ...
      <p className="text-[10px] text-[#251045]/60 leading-snug">{guardrail.micro}</p>
      ...
      {guardrail.next && (
        <div className="rounded-xl border border-[#D6CFE4] bg-[#F1EDF7] p-3">
          <div className="flex items-center justify-between gap-2" style={{ filter: 'blur(1.2px)', opacity: 0.65 }}>
            <span className="text-[11px] font-extrabold text-[#251045]">
              {guardrail.next.label}
            </span>
            <Lock className="h-3.5 w-3.5 text-[#251045]" />
          </div>
          <p className="mt-1 text-[10px] text-[#251045]/70 leading-snug" style={{ filter: 'blur(1.2px)', opacity: 0.7 }}>
      ...
      {attempt && (
        <div className="rounded-xl border border-[#D6CFE4] bg-[#F1EDF7] p-2.5">
          {attempt.ok ? (
            <p className="text-[10px] text-[#251045] flex items-center gap-1.5">
              <Check className="h-3 w-3" /> Tier changed.
            </p>
          ) : (
            <>
              <p className="text-[10px] font-bold text-[#251045] leading-snug">
                {attempt.detail ?? 'The upgrade could not complete yet.'}
              </p>
              {attempt.unlocks && (
                <p className="mt-1 text-[9px] text-[#251045]/60 leading-snug">
