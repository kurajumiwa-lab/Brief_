...
export function startupDiagnostics({ store, capabilities }) {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (!process.env.MPESA_CALLBACK_SECRET && capabilities.payments?.configured) {
      problems.push('payments are configured but MPESA_CALLBACK_SECRET is unset: callbacks will be rejected');
    }
  }
